...
//echo "<br/><br/><br/><pre>";print_r($searchedfields);
...
$quatersArray = $columnsToBeShownAsGraph = array();
...
<section id="container">
 ...
 <div class="main-cell">	
		...
		?>
			...
			<?php if(isset($timeIntervalSettings) && ($timeIntervalSettings['time_format'] == 'SQ-SY-EQ-EY' || $timeIntervalSettings['time_format'] == 'SY-EY')){ 	?>
			<!-- LINE PIE BAR CHART ------------------------->
			<ul class="submenu">
			...
			<div style="margin: 15px;width: 100%;overflow-x:scroll;<?php if((isset($_GET['graph']) && $_GET['graph']!='gridview')){  ?> display:none; <?php } ?>">
			<table cellspacing="0" cellpadding="5" border="1" class="  collapse" id="">
				...
				if(isset($columns['quatersascolumns']) && isset($columns['yearsasrows']) && isset($timeIntervalSettings) && ($timeIntervalSettings['time_format'] == 'SQ-SY-EQ-EY' || $timeIntervalSettings['time_format'] == 'SY-EY')){		// When data is as year q1, q2, q3, q4
					...
					foreach($searchedData as $keyData => $rowData){ 
						...
						if(array_key_exists('Area', $rowData) && array_key_exists('Category', $rowData)){
			...
			<?php if((isset($_GET['graph']) && $_GET['graph']=='linegraph') && isset($timeIntervalSettings) && ($timeIntervalSettings['time_format'] == 'SQ-SY-EQ-EY' || $timeIntervalSettings['time_format'] == 'SY-EY') ){ 	?>
			<div id="line_chart">
				...
				<div style='height:100px;overflow-y:scroll;'>
					...
					foreach($area_chunk as $keyM => $areas){
						...
						foreach($areas as $key=> $area){
							echo "<td><div style='border: 1px solid #000000;float:left; margin-right: 5px;height:10px;width:10px;background:".$colorsarray[$key]."'>&nbsp;</div><div style='float:left;width:92%'>".$area."</div></td>";
						}
						echo "</tr>";
					}
					echo "</table>";
				...
				<script>
					window.onload = function ()
					{
						...
						//line.Set('chart.key.halign', "right");

						//line.Set('chart.key.position.x', 0);
						//line.Set('chart.key.position.y', 0);
