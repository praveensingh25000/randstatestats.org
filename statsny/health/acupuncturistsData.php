...
* @live URL: http://tx.rand.org/stats/health/acupuncturists.html
********************************************/
...
global $db;
if(!isset($_SESSION['acupuncturists']) || (isset($_SESSION['acupuncturists']) && !isset($_SESSION['acupuncturists']['dbid']))){
	header('location: acupuncturists.php');
}
$searchedfields = $_SESSION['acupuncturists'];

$tablesname = "accup";
$tablesnamecat = "accup_cats";
...
$area = implode(',', $searchedfields['Area']);
$category = implode(',', $searchedfields['Category']);

$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and accup.area in (".$area.") and accup.category IN (".$category.") and txplaces.areacode IN (".$states.") ".$joinStr." limit 4000";
