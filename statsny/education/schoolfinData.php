...
if(!isset($_SESSION['schoolfin']) || (isset($_SESSION['schoolfin']) && !isset($_SESSION['schoolfin']['dbid']))){
	header('location: schoolfin.php');
}

$searchedfields = $_SESSION['schoolfin'];
...
$cat2 = '';
foreach($searchedfields['fundtype'] as $keyCat => $catname){
	$cat2 .= "'".$catname."',";
}
$cat2 = substr($cat2, 0, -1);

$area = '';
foreach($searchedfields['areacode'] as $keyCat => $areacode){
	$area .= "".(int)$areacode."000,";
}
...
$category = implode(',', $searchedfields['catcode']);

$sql = "select ".$columnstobefetched." from schoolfin, schoolfin_cats, counties_districts where 1 ".$fieldsStrWhere." and ".$tablesname.".CDS in (".$area.") and ".$tablesnamecat.".catcode in (".$category.") and ".$tablesname.".Cat2 in (".$cat2.") and ".$tablesname.".FinanceType = '".$searchedfields['accounttype']."' ".$joinStr." and ".$tableDistricts.".distcode in (".$area1.") limit 4000";
...
<section id="container">
 ...
 <div class="main-cell">	
		...
		?>
			...
			<?php if($dbsource!=''){ ?>
				<p><strong>Data Source: </strong><?php if(isset($db_datasourcelink) && $db_datasourcelink != ''){ ?><a target="_blank" href="<?php echo $db_datasourcelink; ?>"><?php }?><?php echo stripslashes($dbsource); ?><?php if(isset($db_datasourcelink) && $db_datasourcelink != ''){ ?></a><?php } ?>
				</p>
			<?php } ?>
