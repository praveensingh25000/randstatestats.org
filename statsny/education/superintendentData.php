...
if(!isset($_SESSION['superintendent']) || (isset($_SESSION['superintendent']) && !isset($_SESSION['superintendent']['dbid']))){
	header('location: superintendent.php');
}

$searchedfields = $_SESSION['superintendent'];
...
if(!empty($searchedfields['cds'])){
	$cdsAll = $cdsStr='';
	foreach($searchedfields['cds'] as $cdss){
		$cdsAll.="'".$cdss."'".',';
	}
	$cdsStr = substr($cdsAll, 0, -1);
}
...
$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".CDS in (".$cdsStr.") and ".$tablesname.".Category IN (".$categoryStr.") ".$joinStr." limit 4000";
