...
#live Site URL For This Page: http://tx.rand.org/stats/education/enrollrace.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: enrollraceData.php');
		exit;
	}
}
...
$resultData = $admin->getTableDataUniversal($tableCounties , ' order by areaname');
$allCounties = $dbDatabase->getAll($resultData);

foreach($allCounties as $key => $value){
	if($value['areacode']!=0){
		if(strlen($value['areacode'])==1){
			$value['areacode'] = "00".$value['areacode'];
		} else if(strlen($value['areacode'])==2){
			$value['areacode'] = "0".$value['areacode'];
		}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="enrollrace">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_school_enrollment'])){ echo $lang['lbl_select_school_enrollment']; } ?></span>&nbsp;&nbsp;
				...
				$(document).ready(function() {	
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/stats/education/enrollraceAjax.php',
								type: 'post',
