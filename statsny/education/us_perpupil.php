...
#live Site URL For This Page: http://tx.rand.org/stats/education/us_perpupil.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: us_perpupilData.php');
		exit;
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action=""  novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="us_perpupil">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_state_perpupil'])){ echo $lang['lbl_select_state_perpupil']; } ?></span>&nbsp;&nbsp;
						<?php if(isset($lang['lbl_state_perpupil'])){ echo $lang['lbl_state_perpupil']; } ?>
					</p>
					<div class="table-div">
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_specify_agency_type_perpupil'])){ echo $lang['lbl_specify_agency_type_perpupil']; } ?></span>&nbsp;&nbsp;
						<?php if(isset($lang['lbl_optional_perpupil'])){ echo $lang['lbl_optional_perpupil']; } ?>
					</p>
					<div class="table-div">
				...
				$(document).ready(function() {	
					...
					$('#level').change(function(){
						...
						if(states!='' && states != "NULL"){
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsny/education/us_perpupil_ajax.php',
								type: 'post',
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onDelete: function (item) {
							...
							jQuery('#submitButtons').hide();
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsny/education/us_dist_finances_ajax.php',
								type: 'post',
								data: 'dbid=<?php echo $dbid; ?>&states='+states+'&level='+level,
								success: function(dataresult){
									//jQuery('#citiesDataLoad').html('');
