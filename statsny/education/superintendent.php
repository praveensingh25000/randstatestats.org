...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: superintendentData.php');
		exit;
	}
}
...
foreach($stateToName as $key => $value){
	$stateToNameArray[$value['County']]=array('id' => $value['County_Code'], 'name' => ucwords($value['County']));
}

//echo "<pre>";print_r($stateToNameArray);die;
				
foreach($stateToNameArray as $key => $value){
	if($value['name']!= null)
	$filtervaluesJsonArray[] = array('id' => $value['id'], 'name' => ucwords($value['name']));
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>		
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="superintendent">
				<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_select_county_list_districts_schools'])){ echo $lang['lbl_select_county_list_districts_schools']; } ?> </span>
						</p>
						<div class="table-div">
					...
					$(document).ready(function() {							
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								...
								jQuery.ajax({
									url: URL_SITE+'/stats/education/superintendentAjax.php',
									type: 'post',
