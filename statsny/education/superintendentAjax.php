...
//echo "<pre>";print_r($totalallCategoryDetail);die;
...
if(!empty($totalallCategoryDetail) && !empty($relatedCounty)) {
	...
	foreach($relatedCounty as $stateKey => $stateAlpaCode) {
		...
		$relatedCountyDetail = $admin->getRowUniversal($tablesnamearea, $column='School_Code', $stateKey);

		$lblcities = '';
		if(isset($lang['lbl_county'])){ 
			
			$lblcities = $lang['lbl_county'];
			$lblcities = str_replace("#CITY#", $relatedCountyDetail['County'], $lblcities); 
		} 
