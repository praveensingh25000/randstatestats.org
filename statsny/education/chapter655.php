...
#live Site URL For This Page: http://ny.rand.org/stats/education/chapter655.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: chapter655Data.php');
		exit;
	}
}
...
$resultData = $admin->getDistinctColumnValuesUniversal($tablesnameareas , 'areacounty');
$allCounties = $dbDatabase->getAll($resultData);

$already = array();
foreach($allCounties as $key => $value){
	if(!in_array($value['areacounty'], $already))
	$filtervaluesJsonArray[] = array('id' => $value['areacounty'], 'name' => ucfirst(strtolower($value['areacounty'])));
	
	$already[] = $value['areacounty'];
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action=""  novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="chapter655">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_county_district'])){ echo $lang['lbl_select_county_district']; } ?></span>&nbsp;&nbsp;
				...
				<div class="form-div">
					<p>
					   <span class="choose">Select data category</span>&nbsp;&nbsp;
						
					</p>
					<div class="table-div">
						<input type="radio" value="1" name="type" checked="">&nbsp;District Summary Statistics<br>
						<input type="radio" value="2" name="type">&nbsp;District Test Scores<br>
						<input type="radio" value="3" name="type">&nbsp;Regents Examinations<br>
					</div>
				  </div>
				...
				$(document).ready(function() {	
					$('input[name=type]').change(function(){
						...
						jQuery.ajax({
							url: '<?php echo URL_SITE; ?>/statsny/education/chapter655Ajax.php',
							type: 'post',
							data: 'dbid=<?php echo $dbid; ?>&states='+states+"&cat="+selValue,
