...
if(!isset($_SESSION['staffsalary']) || (isset($_SESSION['staffsalary']) && !isset($_SESSION['staffsalary']['dbid']))){
	header('location: staffsalary.php');
}

$searchedfields = $_SESSION['staffsalary'];
...
$category = '';
foreach($searchedfields['category'] as $keyCat => $categorycode){
	$category .= "'".$categorycode."',";
}
$category = substr($category, 0, -1);

$cat2 = '';
foreach($searchedfields['cat2'] as $keyCat => $cat2code){
	$cat2 .= "'".$cat2code."',";
}
$cat2 = substr($cat2, 0, -1);

$sql = "select ".$columnstobefetched." from staffsalary2, ".$tableDistricts." where 1 ".$fieldsStrWhere." and ".$tablesname.".CDS in (".$area.") and ".$tablesname.".Category in (".$category.") and ".$tablesname.".Cat2 in (".$cat2.") ".$joinStr." and ".$tableDistricts.".distcode in (".$area1.") limit 4000";
