...
if(!isset($_SESSION['leplang']) || (isset($_SESSION['leplang']) && !isset($_SESSION['leplang']['dbid']))){
	header('location: leplang.php');
}

$searchedfields = $_SESSION['leplang'];
...
$cat1 = '';
foreach($searchedfields['cat1'] as $keyCat => $catn){
	$cat1 .= "'".$catn."',";
}
$cat1 = substr($cat1, 0, -1);

$area = '';
foreach($searchedfields['area'] as $keyCat => $areacode){
	$area .= "'".$areacode."',";
}
$area = substr($area, 0, -1);


$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Category in (".$category.") and ".$tablesname.".Cat1 in (".$cat1.") and ".$tablesname.".Area in (".$area.") limit 4000";
