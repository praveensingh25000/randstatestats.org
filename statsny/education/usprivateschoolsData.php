...
if(!isset($_SESSION['usprivateschools']) || (isset($_SESSION['usprivateschools']) && !isset($_SESSION['usprivateschools']['dbid']))){
	header('location: usprivateschools.php');
}

$searchedfields = $_SESSION['usprivateschools'];
...
$area = $areaAll = $areastr = '';
if(!empty($searchedfields['areacode'])) {
	foreach($searchedfields['areacode'] as $keyarea => $area){
		$areaAll .= "'".$area."',";
	}
	$areastr = substr($areaAll, 0, -1);
}

if(isset($searchedfields['sector']) && $searchedfields['sector']!=''){
	$sector = $searchedfields['sector'];
	$fieldsStrWhere .= " and ".$tablesname.".SchoolType = '".$sector."'";
}

$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area IN (".$areastr.") and ".$tablesname.".Category IN (".$category.") ".$joinStr." limit 4000";
