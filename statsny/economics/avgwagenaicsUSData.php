...
$tablesnamestate = "us_states";
$tablesnamearea = "fips";

$admin = new admin();

if(!isset($_SESSION['avgwagenaicsUS']) || (isset($_SESSION['avgwagenaicsUS']) && !isset($_SESSION['avgwagenaicsUS']['dbid']))){
	header('location: avgwagenaicsUS.php');
}

$searchedfields = $_SESSION['avgwagenaicsUS'];
...
$areasAll = $areaStr = $cat1All = $cat1Str = $cat2Str = $cat2All = $cat3All = $cat3Str = $categoryAll = $categoryStr = $stateStr = $stateStrAll = '';
...
if(isset($searchedfields['cat1']) && !empty($searchedfields['cat1'])){	
	$cat1All.="'".$searchedfields['cat1']."'".',';
	$cat1Str = substr($cat1All, 0, -1);
	$cat1Str=$tablesname.".Cat1 in (".$cat1Str.") and ";
}

if(!empty($searchedfields['cat2'])){	
	foreach($searchedfields['cat2'] as $cat2s){
		$cat2All.="'".$cat2s."'".',';
	}
	$cat2Str = substr($cat2All, 0, -1);
}

if(!empty($searchedfields['cat3'])){	
	foreach($searchedfields['cat3'] as $cat3s){
		$cat3All.="'".$cat3s."'".',';
	}
	$cat3Str = substr($cat3All, 0, -1);
}
...
//$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area in (".$areaStr.") and ".$tablesname.".Cat1 in (".$cat1Str.") and ".$tablesname.".Cat2 in (".$cat2Str.") and ".$tablesname.".Cat3 in (".$cat3Str.")  and ".$tablesname.".Category in (".$categoryStr.") ".$joinStr." limit 4000";
$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area in (".$areaStr.")and  ".$cat1Str."".$tablesname.".Cat2 in (".$cat2Str.") and ".$tablesname.".Cat3 in (".$cat3Str.")  and ".$tablesname.".Category in (".$categoryStr.") ".$joinStr." limit 4000";
