...
* @live Site URL For This Page: http://ny.rand.org/stats/economics/avgwagenaicsUS.html
********************************************/
...
$allCategoryDetail_res  = $admin->getTableDataUniversal($tablesnamestate);
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="">
				...
				<div class="form-div">
					<p>
					   <span class="choose">Please choose one or more states:</span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
				...
				<div class="form-div">
					<p>
					   <span class="choose">Choose a type of ownership.</span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
						<?php foreach($ownershipAll as $key => $detailRow){ ?>
						<input type="checkbox" value="<?=$detailRow['Cat2']?>" name="cat2[]"/>&nbsp;<?=$detailRow['Cat2']?><br/>
						<?php } ?>
					</div>
				  </div>
				
				<?php
				$sql = "select DISTINCT( Cat1) from avgwage2US order by Cat1";
				$resultOwnership = $dbDatabase->run_query($sql, $dbDatabase->conn);
				$ownershipAll = $dbDatabase->getAll($resultOwnership);	
				$chunk = round(count($ownershipAll)/2);
				$ownershipAll = array_chunk($ownershipAll ,$chunk);
				?>
				<div class="form-div">
					<p>
					   <span class="choose">Choose an Industry Sector.</span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
						<table>
							<tr>
							<?php foreach($ownershipAll as $key => $detailRowAll){ ?>
								<td>
								<?php foreach($detailRowAll as $key1 => $detailRow){ ?>
									<input type="radio" value="<?=$detailRow['Cat1']?>" name="cat1"/>&nbsp;<?=$detailRow['Cat1']?><br/>
								<?php } ?>
								</td>
							<?php } ?>
							</tr>
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsny/economics/avgwagenaicsUS_ajax.php',
								type: 'post',
						...
						onDelete: function (item) {
							...
							jQuery('#submitButtons').hide();
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsny/avgwagenaicsUS_ajax.php',
								type: 'post',
								data: 'states='+states,
								success: function(dataresult){
									//jQuery('#citiesDataLoad').html('');
