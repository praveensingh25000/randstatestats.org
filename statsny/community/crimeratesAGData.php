...
if(!isset($_SESSION['crimeratesAG']) || (isset($_SESSION['crimeratesAG']) && !isset($_SESSION['crimeratesAG']['dbid']))){
	header('location: crimeratesAG.php');
}

$searchedfields = $_SESSION['crimeratesAG'];
...
if(!empty($searchedfields['us_states'])){
	...
	if(!empty($searchedfields['include_state_totals'])){	
		$stateStr .=",'".$searchedfields['include_state_totals']."'";
		$fieldsStrWhere.=" and ".$tablesname.".area in (".$stateStr.") ";
	} else {
		$fieldsStrWhere.=" and ".$tablesname.".area in (".$stateStr.") ";
	}	
}
...
if(!empty($searchedfields['cat1'])){
	$cat2All = $cat2Str='';
	foreach($searchedfields['cat1'] as $cat2s){
		$cat2All.="'".$cat2s."'".',';
	}
	$cat2Str = substr($cat2All, 0, -1);

	$fieldsStrWhere.=" and ".$tablesname.".cat1 IN (".$cat2Str.") ";
}
