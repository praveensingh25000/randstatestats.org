...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: crimeratesAGData.php');
		exit;
	}
}
...
$stateToName_res = $admin->getDistinctColumnValuesUniversal($tablesname,$column='area',$columns = "",$limit ='');
$stateToName = $dbDatabase->getAll($stateToName_res);

foreach($stateToName as $key => $value){
	
	if($value['area']!='' && $value['area']!=NULL && $value['area']!='New York State'){
		
		if(!is_numeric(stripslashes($value['area'])) && preg_replace('/[^a-zA-Z0-9_ -]/s', '', $value['area'])) {	
			$filtervaluesJsonArray[] = array('id' => $value['area'], 'name' => $value['area']);
			$filtervaluesStr .= '<p>'.str_replace("'","",$value['area'])."</p>";	
		}
	}
}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="crimeratesAG">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_Please_choose_one_or_more_counties'])){ echo $lang['lbl_Please_choose_one_or_more_counties']; } ?></span>
					</p>
					<div class="table-div">
						<div id="" class="left">
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsny/community/crimeratesAGAjax.php',
								type: 'post',
