...
if(!isset($_SESSION['us_hatecrimes']) || (isset($_SESSION['us_hatecrimes']) && !isset($_SESSION['us_hatecrimes']['dbid']))){
	header('location: us_hatecrimes.php');
}

$searchedfields = $_SESSION['us_hatecrimes'];
...
$usstatesArray = explode(';', $searchedfields['us_states']);
$usstates = '';

foreach($usstatesArray as $keyArea => $state){
	$usstates .= "'".$state."',";
}

$usstates = substr($usstates, 0, -1);

$category = '';
foreach($searchedfields['cat1'] as $keyCat => $catname){
	$category .= "'".$catname."',";
}
$category = substr($category, 0, -1);

$cats = '';
foreach($searchedfields['areacode'] as $keyCat => $cat){
	$cats .= "'".$cat."',";
}
...
$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area in (".$usstates.") and ".$tablesname.".Cat1 in (".$category.") and ".$tablesname.".Category In (".$cats.") ".$joinStr." limit 4000";
