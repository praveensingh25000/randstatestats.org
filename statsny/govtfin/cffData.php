...
if(!empty($databaseDetail)){
	...
	if(isset($searchedfields['syear']) && isset($searchedfields['eyear'])){
		...
		if(isset($columns['yearsascolumns'])){
...
if(!empty($searchedfields['all'])){
	$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$joinStr." limit 1000";
} else {
...
<section id="container">
	 ...
	 <div class="main-cell">
		...
		if(count($searchedData)>0) {
			...
			<?php if($dbsource!=''){ ?>
				<p><strong>Data Source: </strong><?php if(isset($db_datasourcelink) && $db_datasourcelink != ''){ ?><a target="_blank" href="<?php echo $db_datasourcelink; ?>"><?php }?><?php echo stripslashes($dbsource); ?><?php if(isset($db_datasourcelink) && $db_datasourcelink != ''){ ?></a><?php } ?></p>
			<?php } ?>
