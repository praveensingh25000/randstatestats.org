...
foreach($totalallCategoryDetailArray as $stateKey => $value) {
	$totalallcffagencyDetail[$value['agency_name']] = array('id' => $value['agency_code'], 'name' => $value['agency_name']);
}
...
//echo "<pre>";print_r($totalallcffagencyDetail);echo "</pre>"; die;
	...
	if(!empty($totalallcffagencyDetail)){
		...
		$data .= '<div class="form-div">
					...
					foreach($totalallcffagencyDetail as $categoryDetail) { 
						$data.='<tr><td><input type="checkbox" value="'.$categoryDetail['id'].'" name="agency[]" />&nbsp;&nbsp;<label for="C1">'.ucwords($categoryDetail['name']).'</label></td></tr>';
					}
