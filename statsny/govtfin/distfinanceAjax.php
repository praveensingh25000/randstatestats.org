...
* Dependent on: statsny/education/distfinance.php
********************************************/
...
if(isset($_REQUEST['states']) && $_REQUEST['states']!=''){
	...
	foreach($statesArray as $stateKey => $stateAlpaCode){
		...
		$columnsArray = array('cdscounty' => $stateAlpaCode);
		
		$dataSqlAllResult = $admin->searchLikeUniversalArray($tablesnamecounties , $columnsArray, 'order by cdsname');
		...
		if(mysql_num_rows($dataSqlAllResult)>0){
			...
			while($areaDetail = mysql_fetch_assoc($dataSqlAllResult)){
				if(!in_array($areaDetail['cdsname'], $alreadyArray)){
					$data .= '<option value="'.$areaDetail['cdscode'].'">'.$areaDetail['cdsname'].'</option>';	
				}
				$alreadyArray[$areaDetail['cdscode']] = $areaDetail['cdsname'];
			}
