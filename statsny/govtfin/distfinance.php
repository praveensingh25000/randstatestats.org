...
#live Site URL For This Page: http://ny.rand.org/stats/education/distfinance.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: distfinanceData.php');
		exit;
	}
}
...
$resultData = $admin->getDistinctColumnValuesUniversal($tablesnamecounties , 'cdscounty');
$allCounties = $dbDatabase->getAll($resultData);

foreach($allCounties as $key => $value){
	$filtervaluesJsonArray[] = array('id' => $value['cdscounty'], 'name' => ucfirst(strtolower($value['cdscounty'])));
}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="distfinance">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_school_districts_performance'])){ echo $lang['lbl_select_school_districts_performance']; } ?></span>&nbsp;&nbsp;
				...
				$(document).ready(function() {	
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsny/education/distfinanceAjax.php',
								type: 'post',
