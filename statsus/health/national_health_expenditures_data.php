...
if(!isset($_SESSION['national_health']) || (isset($_SESSION['national_health']) && !isset($_SESSION['national_health']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: national_health_expenditures.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: national_health_expenditures.php');
		exit;
	}
}

$searchedfields = $_SESSION['national_health'];
...
if(!empty($searchedfields['category'])){
	$catAll = $cat1Str='';
	$categories = explode(';', $searchedfields['category']);
	foreach($categories as $cat1s => $cats){
		$catAll.="'".$cats."'".',';
	}
	$catAll = substr($catAll, 0, -1);

	$fieldsStrWhere .= 	" and ".$tablesname.".category in (".$catAll.") ";
}

if(!empty($searchedfields['source'])){
	$sourceStr = '';
	$sources = explode(';', $searchedfields['source']);
	foreach($sources as $soukey => $sour){
		$sourceStr.="'".$sour."'".',';
	}
	$sourceStr = substr($sourceStr, 0, -1);
	$fieldsStrWhere .= 	" and ".$tablesname.".source in (".$sourceStr.") ";
}

if(!isset($searchedfields['percapita'])){
	$fieldsStrWhere .= 	" and ".$tablesname.".cat3 = 'Not Per Capita' ";
}
