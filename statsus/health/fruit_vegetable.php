...
if(isset($_POST['getresults'])){
	...
	$_SESSION['fruit_vegetable'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: fruit_vegetableData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: fruit_vegetableData.php');
		exit;
		}
	}
}
...
$allCategory_res  = $admin->getDistinctColumnValuesFruits($tablesname , $column='Cat', $category="Fruit", $limit = "");
$allFruits = $dbDatabase->getAll($allCategory_res);

foreach($allFruits as $key => $value){
...
//selecting all vegetables
$allregion_res  = $admin->getDistinctColumnValuesFruits($tablesname , $column='Cat', $category="Vegetable", $limit = "");
$allVegetables = $dbDatabase->getAll($allregion_res);

foreach($allVegetables as $key => $value){
	$filtervaluesJsonArray[] = array('id' => trim($value['Cat']), 'name' => trim($value['Cat']));
}
$filterstatetonameJson = json_encode($filtervaluesJsonArray);

//echo "<pre>";print_r($filtercategoryjsonArray);echo "</pre>";
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="fruit_vegetable">
				
					
				<div id="search_cat_all_data_div" style="" class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_enter_fruit'])){ echo $lang['lbl_enter_fruit']; } ?> </span>
					</p>
					<input class="required" type="text" id="search_criteria_duplicates_fruit" name="Cat1"/>
				</div>
				  ...
				  <div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_enter_vegetable'])){ echo $lang['lbl_enter_vegetable']; } ?> </span>
					</p>
					<div class="table-div">
						<input class="required" type="text" id="search_criteria_duplicates_vegi" name="Cat2"/>
						</div>
				  </div>
				 <!-- /form -->
				...
				jQuery(document).ready(function() {						
					...
					jQuery("#search_criteria_duplicates_fruit").tokenInput(<?php echo $filtercategoryJson; ?>, { preventDuplicates: true		
					});

					jQuery("#search_criteria_duplicates_vegi").tokenInput(<?php echo $filterstatetonameJson; ?>, { preventDuplicates: true	
					});					
				});				
				</script>
