...
$tablesname	= "per_capita_consumption_of_fruits_and_vegetables";

if(!isset($_SESSION['fruit_vegetable']) || (isset($_SESSION['fruit_vegetable']) && !isset($_SESSION['fruit_vegetable']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: fruit_vegetable.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: fruit_vegetable.php');
		exit;
	}
}

$searchedfields = $_SESSION['fruit_vegetable'];
...
$CatArray1 = explode(';',$searchedfields['Cat1']);
$CatArray2 = explode(';',$searchedfields['Cat2']);
$CatArray = array_merge($CatArray1,$CatArray2);
if(!empty($CatArray)){
	$catStr = $catStrAll ='';
