...
$tablesname					= "per_capita_nat_health_ins";
...
if(isset($_POST['getresults'])){
	...
	$_SESSION['national_health'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: national_health_expenditures_data.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: national_health_expenditures_data.php');
		exit;
		}
	}
}
...
$allCategoryDetail_res  = $admin->getDistinctColumnValuesUniversal($tablesname , 'category');
$already = array();

while($value = mysql_fetch_assoc($allCategoryDetail_res)){
	$filtervaluesJsonArray[] = array('id' => $value['category'], 'name' => $value['category']);
}

$filtervaluesJson = json_encode($filtervaluesJsonArray);

$allSourceDetail_res  = $admin->getDistinctColumnValuesUniversal($tablesname , 'source');

$filtervaluesSourceJsonArray = array();
while($value = mysql_fetch_assoc($allSourceDetail_res)){
	if(!in_array($value['source'], $filtervaluesJsonArray))
	$filtervaluesSourceJsonArray[] = array('id' => $value['source'], 'name' => $value['source']);
}

$filtervaluesSourceJson = json_encode($filtervaluesSourceJsonArray);
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
					...
					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_enter_cat_expenditure'])){ echo $lang['lbl_enter_cat_expenditure']; } ?></span>
						</p>
						<div class="table-div">
					...
					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_enter_source_of_funds'])){ echo $lang['lbl_enter_source_of_funds']; } ?></span>
						</p>
						<div class="table-div">
							<input type="text" id="search_criteria_duplicates_source" name="source"/>
						</div>
						<script type="text/javascript">
						$(document).ready(function() {							
					
							$("#search_criteria_duplicates_source").tokenInput(<?php echo $filtervaluesSourceJson; ?>, { preventDuplicates: true });
						});
						</script>
					</div>
					...
					<div class="form-div">
						<div class="table-div">
							<input type="checkbox" id="percapita" name="percapita"/>&nbsp;<?php if(isset($lang['lbl_include_per_capita_exp'])){ echo $lang['lbl_include_per_capita_exp']; } ?>
						</div>
					</div>
