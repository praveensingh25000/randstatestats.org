...
if(isset($_POST['getresults'])){
	...
	$_SESSION['consumer_prices_urban'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: consumer_prices_urbanData.php?dbc='.$dbnameurl.'');			
			exit;
		} else {
		header('location: consumer_prices_urbanData.php');
		exit;
		}
	}
}
...
//For Seasonal Detail
$seasonaldetailages_res  = $admin->getDistinctColumnValuesUniversal($tablesname , $column='seasonal', $columns = "", $limit = "");
$totalSeasonalDetail = $dbDatabase->getAll($seasonaldetailages_res);
foreach($totalSeasonalDetail as $key => $value){
	$popcontentSeasonal .= '<p>'.$value['seasonal']."</p>";
	$filterSeasonalvaluesJsonArray[] = array('id' => $value['seasonal'], 'name' => $value['seasonal']);
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="<?php echo $URL_SITE.$_SERVER['PHP_SELF']?>" novalidate="novalidate">
					...
					<!-- form -->
					<input type="hidden" name="session_setter" value="consumer_prices_urban">
					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_enter_seasonal_code'])){ echo $lang['lbl_enter_seasonal_code']; } ?><!-- &nbsp;&nbsp;<a id="viewallcat1" href="javascript:;" onclick="javascript: return popup_window('<?php echo $popcontentSeasonal; ?>');">See a list</a> --></span>
						</p>
						<div class="table-div">
							<div class="left">
								<table>
									<tbody>
										...
										foreach($filterSeasonalvaluesJsonArray as $values) { ?>
											<td>
												<input class="required" type="checkbox" id="search_criteria_duplicates_cat1" value="<?php echo $values['id']?>" name="seasonal[]" />&nbsp;<b><?php echo $values['name']?></b>
											<td>
										<?php } ?>
										</tr>
									<tbody>
					...
					<div class="form-div">
						...
						<div class="table-div">
							<div class="left">
								<input class="required" type="radio" id="search_cpiu" name="cpiu" value="percent" />&nbsp;&nbsp;<b>CPI-W Index</b>
							</div>
