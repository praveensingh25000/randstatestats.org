...
$tablesname		   = "consumer_prices_urban_consumers";
$tablesnameareas   = "consumer_prices_urban_consumers_area";
$tablesnameitems   = "consumer_prices_urban_consumers_item";

$admin = new admin();
$user  = new user();

if(!isset($_SESSION['consumer_prices_urban']) || (isset($_SESSION['consumer_prices_urban']) && !isset($_SESSION['consumer_prices_urban']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: consumer_prices_urban.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: consumer_prices_urban.php');
		exit;
	}
}

$searchedfields = $_SESSION['consumer_prices_urban'];
...
if(!empty($searchedfields['seasonal'])){	
	$cat1array = $searchedfields['seasonal'];
	foreach($cat1array as $cat1s){
		$cat1All.="'".$cat1s."'".',';
	}
	$cat1Str = substr($cat1All, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".seasonal IN (".$cat1Str.") ";
}
...
if(isset($searchedfields['cpiu']) && $searchedfields['cpiu']=='percent') {
	$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere."  ".$joinStr." order by ".$tablesname.".year limit 4000";
	
	$_SESSION['query'] = $sql ;

} else {

	$columnstobefetched .= ",".$tablesnameareas.".area_code as area_code, ".$tablesname.".items as item_code";
...
<?php } else { 
	...
	if(!empty($searchedData)) {
	   ...
	   foreach($searchedData as $keyData => $values2) {
			...
			$Status     = $values2['Seasonal'];
			
			 $Jan =  $Feb = $Mar = $Apr = $May = $June = $July = $Aug = $Sep = $Oct = $Nov = $Dec = $Avg = "NA";

			echo $sqlPreviousYear = "SELECT ".$tablesnameareas.".area_name AS `Area` , ".$tablesnameitems.".item_name AS `Item` , ".$tablesname.".seasonal AS `Status` , ".$tablesname.".year AS `Year` , ".$tablesname.".m01 AS 'Jan', ".$tablesname.".m02 AS 'Feb', ".$tablesname.".m03 AS 'Mar', ".$tablesname.".m04 AS 'Apr', ".$tablesname.".m05 AS 'May', ".$tablesname.".m06 AS 'June', ".$tablesname.".m07 AS 'July', ".$tablesname.".m08 AS 'Aug', ".$tablesname.".m09 AS 'Sep', ".$tablesname.".m10 AS 'Oct', ".$tablesname.".m11 AS 'Nov', ".$tablesname.".m12 AS 'Dec', ".$tablesname.".Avg AS `Avg` FROM ".$tablesname.", ".$tablesnameareas.", ".$tablesnameitems." WHERE 1 AND ".$tablesname.".year = '".$year."' AND ".$tablesname.".seasonal IN ('".$Status."') AND ".$tablesname.".areas = '".$areacode."' AND ".$tablesname.".items IN ('".$itemcode."') AND ".$tablesname.".areas = ".$tablesnameareas.".area_code AND ".$tablesname.".items = ".$tablesnameitems.".item_code";
			echo '<br/><br/>';
	   ...
	   <?php } else { ?>		
		...
		<div class="clear"></div>
		<div class="pT20 txtcenter"><b>No Records Found For Search Criteria.</b></div>
