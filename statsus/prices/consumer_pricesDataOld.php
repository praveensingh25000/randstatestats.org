...
if(!empty($searchedfields['cat1'])){	
	$cat1array = explode(';',$searchedfields['cat1']);
...
echo $sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere."  ".$joinStr." limit 4000";
...
<?php } else { 
	...
	if(!empty($searchedData)) {
	   ...
	   foreach($search_array as $key => $values){
		   ...
		   if($key % 2){
			   $search_array1[$key] = $values;
		   } else {
			   $search_array2[$key] = $values;
		   }		   
	   }

	   $Jan =  $Feb = $Mar = $Apr = $May = $June = $July = $Aug = $Sep = $Oct = $Nov = $Dec = 0.0;

	   if(!empty($search_array1) && !empty($search_array1)){
		   foreach($search_array1 as $key1 => $values1){
			   foreach($search_array2 as $key2 => $values2){

				   if(trim($values1['Area']) == trim($values2['Area']) && trim($values1['Item']) == trim($values2['Item']) && trim($values1['Status']) == trim($values2['Status']) && trim($values1['Year'] + 1) == trim($values2['Year'])) {
						
						if(isset($values2['Jan']) && isset($values1['Jan'])){ $Jan = number_format(((($values2['Jan']/$values1['Jan']) - 1)*100),2); }

						if(isset($values2['Feb']) && isset($values1['Feb'])){ $Feb = number_format(((($values2['Feb']/$values1['Feb']) - 1)*100),2); }

						if(isset($values2['Mar']) && isset($values1['Mar'])){ $Mar = number_format(((($values2['Mar']/$values1['Mar']) - 1)*100),2); }

						if(isset($values2['Apr']) && isset($values1['Apr'])){ $Apr = number_format(((($values2['Apr']/$values1['Apr']) - 1)*100),2); }

						if(isset($values2['May']) && isset($values1['May'])){ $May = number_format(((($values2['May']/$values1['May']) - 1)*100),2); }

						if(isset($values2['June']) && isset($values1['June'])){ $June = number_format(((($values2['June']/$values1['June']) - 1)*100),2); }

						if(isset($values2['July']) && isset($values1['July'])){ $July = number_format(((($values2['July']/$values1['July']) - 1)*100),2); }

						if(isset($values2['Aug']) && isset($values1['Aug'])){ $Aug = number_format(((($values2['Aug']/$values1['Aug']) - 1)*100),2); }

						if(isset($values2['Sep']) && isset($values1['Sep'])){ $Sep = number_format(((($values2['Sep']/$values1['Sep']) - 1)*100),2); }

						if(isset($values2['Oct']) && isset($values1['Oct'])){ $Oct = number_format(((($values2['Oct']/$values1['Oct']) - 1)*100),2); }
						if(isset($values2['Nov']) && isset($values1['Nov'])){ $Nov = number_format(((($values2['Nov']/$values1['Nov']) - 1)*100),2); }

						if(isset($values2['Dec']) && isset($values1['Dec'])){ $Dec = number_format(((($values2['Dec']/$values1['Dec']) - 1)*100),2); }

						if(isset($values2['Avg']) && isset($values1['Avg'])){ $Avg = number_format(((($values2['Avg']/$values1['Avg']) - 1)*100),2); }


					   $search_array11[$values1['Year'].'-'.$values2['Year']]['Area'] = $values1['Area'];
					   $search_array11[$values1['Year'].'-'.$values2['Year']]['Item'] = $values1['Item'];
					   $search_array11[$values1['Year'].'-'.$values2['Year']]['Status'] = $values1['Status'];
					   $search_array11[$values1['Year'].'-'.$values2['Year']]['Year'] = $values1['Year'].'-'.$values2['Year'];

					   if(isset($values2['Jan']) && isset($values1['Jan'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Jan']   = round($Jan,1)."%"; }

					   if(isset($values2['Feb']) && isset($values1['Feb'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Feb']   = round($Feb,1)."%"; }

					   if(isset($values2['Mar']) && isset($values1['Mar'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Mar']   = round($Mar,1)."%"; }

					   if(isset($values2['Apr']) && isset($values1['Apr'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Apr']   = round($Apr,1)."%"; }
					   
					   if(isset($values2['May']) && isset($values1['May'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['May']   = round($May,1)."%"; }

					   if(isset($values2['June']) && isset($values1['June'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['June']  = round($June,1)."%"; }

					   if(isset($values2['July']) && isset($values1['July'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['July']  = round($July,1)."%"; }

					   if(isset($values2['Aug']) && isset($values1['Aug'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Aug']   = round($Aug,1)."%"; }

					   if(isset($values2['Sep']) && isset($values1['Sep'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Sep']   = round($Sep,1)."%"; }

					   if(isset($values2['Oct']) && isset($values1['Oct'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Oct']   = round($Oct,1)."%"; }

					   if(isset($values2['Nov']) && isset($values1['Nov'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Nov']   = round($Nov,1)."%"; }

					   if(isset($values2['Dec']) && isset($values1['Dec'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Dec']   = round($Dec,1)."%"; }

					    if(isset($values2['Avg']) && isset($values1['Avg'])){ $search_array11[$values1['Year'].'-'.$values2['Year']]['Avg']   = round($Avg,1)."%"; }
				   }
			   }
		   }
