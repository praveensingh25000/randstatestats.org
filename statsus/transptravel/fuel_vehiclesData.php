...
if(!isset($_SESSION['fuel_vehicles']) || (isset($_SESSION['fuel_vehicles']) && !isset($_SESSION['fuel_vehicles']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: fuel_vehicles.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: fuel_vehicles.php');
		exit;
	}
}

$searchedfields = $_SESSION['fuel_vehicles'];
...
if(!empty($searchedfields['State'])){
	$stateStr = $stateStrAll ='';
	$statesArray = explode(';',$searchedfields['State']);
	foreach($statesArray as $states){
		$stateStrAll.="'".$states."'".',';
	}
	$stateStr = substr($stateStrAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".State in (".$stateStr.") ";
}


if(!empty($searchedfields['fuelType'])){
	$fuelTypeStr = $fuelTypeStrAll ='';
	$fuelTypeArray = explode(';',$searchedfields['fuelType']);
	foreach($fuelTypeArray as $fuelType){
		$fuelTypeStrAll.="'".$fuelType."'".',';
	}
	$fuelTypeStr = substr($fuelTypeStrAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".fuelType in (".$fuelTypeStr.") ";
}

if(!empty($searchedfields['vehicleType'])){
	$vehicleTypeAll = $vehicleTypeStr='';
	foreach($searchedfields['vehicleType'] as $vehicleType){
		$vehicleTypeAll.="'".$vehicleType."'".',';
	}
	$vehicleTypeStr = substr($vehicleTypeAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".vehicleType in (".$vehicleTypeStr.") ";
}


if(!empty($searchedfields['engineConfiguration'])){
	$engineConfigurationAll = $engineConfigurationStr='';
	foreach($searchedfields['engineConfiguration'] as $engineConfiguration){
		$engineConfigurationAll.="'".$engineConfiguration."'".',';
	}
	$engineConfigurationStr = substr($engineConfigurationAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".engineConfiguration in (".$engineConfigurationStr.") ";
}

$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." ".$joinStr." order by ".$tablesname.".Year, ".$tablesname.".State limit 4000";
