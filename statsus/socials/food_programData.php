...
if(!isset($_SESSION['food_program']) || (isset($_SESSION['food_program']) && !isset($_SESSION['food_program']['dbid']))) {
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: food_program.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: food_program.php');
		exit;
	}
}

$searchedfields = $_SESSION['food_program'];
...
$areaAll = $areaStr = $categoryAll = $categoryStr = $programStr = $programAll = '';
...
if(isset($searchedfields['Area']) && $searchedfields['Area']!='') {

	$areaArray = explode(';', $searchedfields['Area']);
	foreach($areaArray as $keyArea => $area){
		$areaAll .= "'".$area."',";
	}
	$areaStr = substr($areaAll, 0, -1);
