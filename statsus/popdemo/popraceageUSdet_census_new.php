...
$tablesnamecat				= "multiraceexport_cats";
...
if(isset($_POST['getresults'])){
	...
	$_SESSION['popraceageUSdet_census_new'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: popraceageUSdet_census_newData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: popraceageUSdet_census_newData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="popraceageUSdet_census_new">
					<div class="form-div">
						<p>
						...
						<div class="table-div">							
							...
							<div class="left">&nbsp;&nbsp;
								<a id="viewall" href="javascript:;" onclick="javascript: return popup_window('<?php echo $popcontent; ?>');">See a list</a>
							</div>			
						<div class="clear"></div>
						</div>
					</div>
					...
					$(document).ready(function() {							
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								...
								jQuery.ajax({
									url: URL_SITE+'/statsus/popdemo/popraceageUSdet_censusAjax_new.php',
									type: 'post',
