...
$tablesnamecounty		= "us_states";

if(!isset($_SESSION['us_births']) || (isset($_SESSION['us_births']) && !isset($_SESSION['us_births']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: us_births.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: us_births.php');
		exit;
	}
}

$searchedfields = $_SESSION['us_births'];
