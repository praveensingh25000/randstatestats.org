...
if(isset($_POST['getresults'])){
	...
	$_SESSION['usmortality_strace'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: usmortality_straceData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: usmortality_straceData.php');
		exit;
		}
	}
}
...
foreach($stateToName as $key => $value){
	...
	$filtervaluesJsonArray[] = array('id' => $value['statename'], 'name' => $value['statename']);
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
					...
					<!-- form -->
					<input type="hidden" name="session_setter" value="usmortality_strace">
					<div class="form-div">
						<p>
					...
					$(document).ready(function() {							
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								...
								jQuery.ajax({
									url: URL_SITE+'/statsus/popdemo/usmortality_straceAjax.php',
									type: 'post',
