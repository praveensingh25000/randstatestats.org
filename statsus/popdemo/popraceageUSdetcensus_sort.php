...
if(isset($_POST['getresults'])){
	...
	$_SESSION['popraceageUSdetcensus_sort'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: popraceageUSdetcensus_sortData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: popraceageUSdetcensus_sortData.php');
		exit;
		}
	}
}
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<div class="bottom-submit">	
					...
					<!-- form -->
					<input type="hidden" name="session_setter" value="popraceageUSdetcensus_sort">
					<div class="form-div">				  
						<h4>
							<span class="">
								<?php if(isset($lang['lbl_largest_by_county'])){ echo $lang['lbl_largest_by_county'];} ?>
							</span>			   
