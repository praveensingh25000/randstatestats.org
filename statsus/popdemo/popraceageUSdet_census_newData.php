...
if(!isset($_SESSION['popraceageUSdet_census_new']) || (isset($_SESSION['popraceageUSdet_census_new']) && !isset($_SESSION['popraceageUSdet_census_new']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: popraceageUSdet_census_new.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location:popraceageUSdet_census_new.php');
		exit;
	}
}

$searchedfields = $_SESSION['popraceageUSdet_census_new'];
...
if(!empty($searchedfields['Cat1'])){
	...
	$fieldsStrWhere .= " and ".$tablesnameages.".age_code IN (".$cat1Str.") ";
}
...
if(!empty($searchedfields['Category'])){	
	...
	$fieldsStrWhere .= " and ".$tablesnamecat.".cat_id IN (".$categoryStr.")  ";
}
