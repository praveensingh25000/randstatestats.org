...
if(isset($_POST['getresults'])){
	...
	$_SESSION['popraceageUSdet'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: popraceageUSdetData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: popraceageUSdetData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="popraceageUSdet">
					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_choose_three_geographic_areas'])){ echo $lang['lbl_choose_three_geographic_areas']; } ?> </span>
						</p>
						<div class="table-div">
							<div class="left">
					...
					$(document).ready(function() {							
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								...
								jQuery.ajax({
									url: URL_SITE+'/statsus/popdemo/popraceageUSdetAjax.php',
									type: 'post',
