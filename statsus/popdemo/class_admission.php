...
$tablesname	= "country_birth_last_residence_and_class_admission";

$databaseCategories = $databaseRelatedDatabases = array();

if(isset($_POST['getresults'])){

	$_SESSION['class_admission'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: class_admissionData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: class_admissionData.php');
		exit;
		}
	}
}
...
$allCategory_res  = $admin->getDistinctColumnValuesUniversal($tablesname , $column='Cat', $columns = "", $limit = "");
$allCategory = $dbDatabase->getAll($allCategory_res);

foreach($allCategory as $key => $value){
	$filtercategoryjsonArray[] = array('id' => trim($value['Cat']), 'name' => trim($value['Cat']));
}
$filtercategoryJson = json_encode($filtercategoryjsonArray);

//selecting all regions
$allregion_res  = $admin->getDistinctColumnValuesUniversal($tablesname , $column='region', $columns = "", $limit = "");
$stateToName = $dbDatabase->getAll($allregion_res);

foreach($stateToName as $key => $value){
	$filtervaluesJsonArray[] = array('id' => trim($value['region']), 'name' => trim($value['region']));
}
$filterstatetonameJson = json_encode($filtervaluesJsonArray);

//echo "<pre>";print_r($filtercategoryJson);echo "</pre>";
//echo "<pre>";print_r($filterstatetonameJson);echo "</pre>";
?>
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="class_admission">
				<div id="region_country_div" class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_please_select_category'])){ echo $lang['lbl_please_select_category']; } ?> </span>
					</p>

					<!-- Region or country of birth -->
					<div class="pT5 pB5">
						<input id="region_country_birth" type="radio" name="Category" value="Country of birth" class="required">&nbsp;&nbsp;&nbsp;
						<span class="choose"><?php if(isset($lang['ibl_region_or_country_of_birth'])){ echo $lang['ibl_region_or_country_of_birth']; } ?> </span>
					</div>
					<div class="clear"></div>

					<!-- Region or country of last residence. -->
					<div class="pT5 pB5">						
						<input id="region_country_residence" type="radio" name="Category" value="Last residence" class="required">&nbsp;&nbsp;&nbsp;						
						<span class="choose"><?php if(isset($lang['lbl_region_or_country_of_last_residence'])){ echo $lang['lbl_region_or_country_of_last_residence']; } ?> </span>
					</div>
					<div class="clear"></div>

					<!-- Or both (all). -->
					<div class="pT5 pB5">						
						<input id="region_country_all" type="radio" name="Category" value="all data" class="required">&nbsp;&nbsp;&nbsp;	
						<span class="choose"><?php if(isset($lang['ibl_region_or_country_of_all'])){ echo $lang['ibl_region_or_country_of_all']; } ?> </span>
					</div>
					<div class="clear"></div>
					
					<div id="search_cat_all_data_div" style="display:none" class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_enter_category'])){ echo $lang['lbl_enter_category']; } ?> </span>
						</p>
						<input class="field_required_div" type="text" id="search_cat_all_data" name="Cat"/>
						<label style="display:none;" for="search_cat_all_data" generated="true" class="error">This field is required.</label>
					</div>
				  ...
				  <div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_enter_region_or_country'])){ echo $lang['lbl_enter_region_or_country']; } ?> </span>
					</p>
					<div class="table-div">
						<input class="field_required_div" type="text" id="search_criteria_duplicates" name="regions" />
					</div>
				  </div>
				 <!-- /form -->
				...
				jQuery(document).ready(function() {					
					...
					jQuery("#search_criteria_duplicates").tokenInput(<?php echo			$filterstatetonameJson; ?>, { preventDuplicates: true					
					});					
						jQuery("#search_cat_all_data").tokenInput(<?php echo $filtercategoryJson; ?>, { preventDuplicates: true					
					});		
						
					jQuery('#region_country_birth, #region_country_residence, #region_country_all').live('click', function() {						
						jQuery('#search_cat_all_data_div').show();
					});

					$("#submit_admision_<?php echo $dbid; ?>").click(function() {					
						var validate_type = jQuery('.token-input-token').html();					
						if(validate_type==null || validate_type==''){
							if(!jQuery("#field_required").hasClass('fieldrequired'))
							jQuery(".field_required_div").after('<label class="fieldrequired error" for="input" generated="true" class="error">This field is required</label>');
							return false;
						} else {
					...
					$("#token-input-search_cat_all_data").click(function() {						
						jQuery(".fieldrequired").hide();
					});						
				...
				<div class="bottom-submit">	
					...
					<div class="submitbtn-div" id="submitButtons" style="">
