...
if(!isset($_SESSION['class_admission']) || (isset($_SESSION['class_admission']) && !isset($_SESSION['class_admission']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: class_admission.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: class_admission.php');
		exit;
	}
}

$searchedfields = $_SESSION['class_admission'];
...
$statesArray = explode(';',$searchedfields['regions']);
if(!empty($statesArray) && isset($statesArray[0]) && $statesArray[0]!=''){	
	$stateStr = $stateStrAll ='';
	foreach($statesArray as $states){
		$stateStrAll.="'".$states."'".',';
	}
	$stateStr = substr($stateStrAll, 0, -1);	
	$fieldsStrWhere .= " and ".$tablesname.".region in (".$stateStr.") ";
	
}

$CatArray = explode(';',$searchedfields['Cat']);
if(!empty($CatArray)){
	$catStr = $catStrAll ='';
	foreach($CatArray as $cats){
		$catStrAll.="'".$cats."'".',';
	}
	$catStr = substr($catStrAll, 0, -1);

	$fieldsStrWhere .= " and ".$tablesname.".Cat in (".$catStr.") ";
}

if(!empty($searchedfields['Category'])){

	if($searchedfields['Category']== 'all data'){
		$fieldsStrWhere .= "";
	} else {
		$fieldsStrWhere .= " and ".$tablesname.".Category in ('".$searchedfields['Category']."') ";
	}
}
...
<section id="container">
	 ...
	 <div class="main-cell">
		...
		if(count($searchedData)>0) {
			...
			<p>&nbsp;</p>
			<p class="fontbld"><b>*</b>=Data withheld to limit disclosure, <b>X</b>=Not applicable.</p>
