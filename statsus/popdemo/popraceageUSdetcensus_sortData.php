...
if(!isset($_SESSION['popraceageUSdetcensus_sort']) || (isset($_SESSION['popraceageUSdetcensus_sort']) && !isset($_SESSION['popraceageUSdetcensus_sort']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: popraceageUSdetcensus_sort.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location:popraceageUSdetcensus_sort.php');
		exit;
	}
}

$searchedfields = $_SESSION['popraceageUSdetcensus_sort'];
...
$searchedDatamainArray = $searchedDatamain = $searchedData = $quatersArray = $monthssArray = $columnsToBeShownAsGraph = $searchedDataArray = array();
...
if(!empty($searchedfields['largest_by_name']) || !empty($searchedfields['largest_by_number'])){
	...
	$fieldsStrWhere .= " and ".$tablesnamecountyarea.".areaname LIKE '%County%' ";
}

$sql = "select ".$columnstobefetched."  from ".$tablesStr." where 1 ".$joinStr." ".$fieldsStrWhere." and ".$tablesname.".Cat1 = 'All ages' and ".$tablesname.".Category = '10' ORDER BY ".$tablesname.".Y".$searchedfields['syear']." DESC LIMIT 4000 ";

$searchedDataResult = $dbDatabase->run_query($sql);
$totalsearchedData  = $dbDatabase->count_rows($searchedDataResult);

if(isset($totalsearchedData) && $totalsearchedData > 0){
	while($searchData = mysql_fetch_assoc($searchedDataResult)){
		$searchedDatamainArray[$searchData['Area']][] = $searchData;
	}
	if(!empty($searchedDatamainArray)){		
		foreach($searchedDatamainArray as $statekey =>$searchArray){
			if(isset($searchArray[0]) && $searchArray[0]!=''){
				$searchedDatamain[] = $searchArray[0];
			}
		} 		
		$searchedData = $_SESSION['query'] = array_slice($searchedDatamain,0,$limitmumber,false);
	}
}
?>
