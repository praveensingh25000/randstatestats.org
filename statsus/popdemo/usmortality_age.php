...
if(isset($_POST['getresults'])){
	...
	$_SESSION['usmortality_age'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: usmortality_ageData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: usmortality_ageData.php');
		exit;
		}
	}
}
...
foreach($stateToName as $key => $value){
	if($value['statename']!='United States'){
	$filtervaluesStr .= '<p>'.trim($value['statename'])."</p>";
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
					...
					<!-- form -->
					<input type="hidden" name="session_setter" value="usmortality_age">
					<div class="form-div">
						<p>
					...
					$(document).ready(function() {							
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								...
								jQuery.ajax({
									url: URL_SITE+'/statsus/popdemo/usmortality_ageAjax.php',
									type: 'post',
