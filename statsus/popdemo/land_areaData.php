...
$nograph =1; // 1 for no graph display,0 for garph display

$tablesname				= "state_and_county_land_area";

if(!isset($_SESSION['land_area']) || (isset($_SESSION['land_area']) && !isset($_SESSION['land_area']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: land_area.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: land_area.php');
		exit;
	}
}

$searchedfields = $_SESSION['land_area'];
...
if(!empty($searchedfields['County'])){
	...
	foreach($searchedfields['County'] as $areas){
		$areasAll.="'".$areas."'".',';
	}
	$areaStr = substr($areasAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".County IN (".$areaStr.") ";
}
