...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: land_areaData.php?dbc='.$dbnameurl.'');			
			exit;
		} else {
		header('location: land_areaData.php');
		exit;
		}
	}
}
...
$totalCategoryDetailArray      = $dbDatabase->getAll($allCategoryDetailages_res);

$statetoname_res  = $admin->getDistinctColumnValuesUniversal($tablesname , $column='State', $columns = "", $limit = "");
$statetName = $dbDatabase->getAll($statetoname_res);

foreach($statetName as $key => $value){
	if($value['State'] != null)
	$filtervaluesJsonArray[] = array('id' => $value['State'], 'name' => $value['State']);
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="<?php echo $URL_SITE.$_SERVER['PHP_SELF']?>" novalidate="novalidate">
					...
					<!-- form -->
					<input type="hidden" name="session_setter" value="land_area">
					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lb_choose_category'])){ echo $lang['lb_choose_category']; } ?>&nbsp;&nbsp;&nbsp;<input id="search_criteria_duplicates_all" type="checkbox" value="Y" name="allow_all" /> All</span>
						   <script type="text/javascript">
							$(document).ready(function() {							
								jQuery("#search_criteria_duplicates_all").click(function(){					
									if(jQuery(this).is(":checked")){
										jQuery(".search_category_disable").attr('disabled', 'true');			
									} else {
										jQuery(".search_category_disable").removeAttr('disabled');
									}
								});
							});
							</script>
						...
						<div class="table-div">
							<?php if(!empty($totalCategoryDetailArray)) {								
								$totalCategoryDetail=array_chunk($totalCategoryDetailArray,2,true);
								?>
								<table width="100%">
									
										<?php foreach($totalCategoryDetail as $keyAll => $valueAll) { ?>
										<tr>
										<?php foreach($valueAll as $key => $value) { ?>
											<td align="left">
												<input type="radio" class="search_category_disable required" name="Category[]" value="<?php echo $value['Category'];?>" />&nbsp;&nbsp;&nbsp;<?php echo $value['Category']?>
											</td>
											<td align="left">&nbsp;</td>
											<td align="left">&nbsp;</td>
											<td align="left">&nbsp;</td>								
					...
					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lb_choose_state'])){ echo $lang['lb_choose_state']; } ?> </span>
						</p>
						<div class="table-div">
					...
					$(document).ready(function() {							
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								...
								jQuery.ajax({
									url: URL_SITE+'/statsus/popdemo/land_areaAjax.php',						
									type: 'post',
					...
					<div class="bottom-submit">
						...
						?>
							<!-- <input type="hidden" name="syear" value="2013">
							<input type="hidden" name="eyear" value="2013">
