...
#live Site URL For This Page: http://statestats.rand.org/stats/community/popdensityUSdet.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: popdensityUSdetData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
...
$popcontent ='';
$allCategoryDetail_res  = $admin->getTableDataUniversal($tablesnamecounty);
$stateToName = $dbDatabase->getAll($allCategoryDetail_res);
foreach($stateToName as $key => $value){
	if($value['statename'] != null){
		$popcontent .= '<p>'.$value['statename']."</p>";
		$filtervaluesJsonArray[] = array('id' => $value['state'], 'name' => $value['statename']);
	}
}
			...
			<form method="post" id="frmPost" name="frmPost" action="">
				...
				<div class="form-div">
					...
					<div class="table-div">
						...
						<div class="left">&nbsp;&nbsp;
							<a id="viewall" href="javascript:;" onclick="javascript: return popup_window('<?php echo $popcontent; ?>');">See a list</a></div>			
						<div class="clear"></div>						
					</div>
				  </div>
				<!-- /form -->
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/community/popdensityUSdetAjax.php',
								type: 'post',
