...
#live Site URL For This Page: http://statestats.rand.org/stats/community/us_hatecrimes.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: us_hatecrimesData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: us_hatecrimesData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="">
				...
				<input type="hidden" name="session_setter" value="us_hatecrimes">
				
				<!-- <div class="form-div">
					<p>
					   <span class="choose">
							<span class="choose"><?php if(isset($lang['lbl_select_state_reporting_agencies'])){ echo $lang['lbl_select_state_reporting_agencies']; } ?></span>
						</span>
					</p>					
				</div> -->
				...
				<div class="form-div">
					<p>
					   <span class="choose">
							<?php if(isset($lang['lbl_select_state_reporting_agencies'])){ echo $lang['lbl_select_state_reporting_agencies']; } ?>
						</span>
					</p>
					<div class="table-div">
				...
				<div class="form-div">
					<p>
					   <span class="choose">
							<?php if(isset($lang['lbl_crimes_agencies_reporting'])){ echo $lang['lbl_crimes_agencies_reporting']; } ?>
						</span>
					</p>
					<div class="table-div">
						<select size="4" name="agtype" id="agtype" class="required">
							<option value="all">List all agencies</option>
							<option value="city">List cities</option>
							<option value="county">List counties</option>
							<option value="total">List State Totals</option>
						</select>						
					</div>
				</div>
				<!-- /form -->
				...
				$(document).ready(function() {
					...
					$('#agtype').change(function(){
						...
						if(states!='' && states != 'NULL'){
							...
							jQuery.ajax({
								url: URL_SITE+'/statsus/community/us_hatecrimes_ajax.php',
								type: 'post',
								data: 'dbid=<?php echo $dbid; ?>&states='+states+'&agtype='+agtype,
							...
							alert("<?php if(isset($lang['lbl_select_state_reporting_agencies'])){ echo $lang['lbl_select_state_reporting_agencies']; } ?>");
						}
					});
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {			preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							if($.trim(agtype)!='' && $.trim(agtype)!='NULL'){							
								...
								alert("<?php if(isset($lang['lbl_crimes_agencies_reporting'])){ echo $lang['lbl_crimes_agencies_reporting']; } ?>");
							} */
						},
