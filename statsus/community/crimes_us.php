...
* @live Site URL For This Page:	http://statestats.rand.org/stats/community/crimes_us.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: crimes_usData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: crimes_usData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="crimes_us">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_state_list_agencies'])){ echo $lang['lbl_select_state_list_agencies']; } ?></span>&nbsp;&nbsp;
						<?php if(isset($lang['lbl_states_crimes'])){ echo $lang['lbl_states_crimes']; } ?>
					</p>
					<div class="table-div">
						<div class="wdthpercent50 left">
						<input type="text" id="search_criteria_duplicates" name="us_states"/></div>
						
						<div class="pL20 left">&nbsp;<a onclick="javascript: return popup_window('<?php echo $strPop; ?>');" href="javascript:;">See a list</a></div>
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/community/crimes_us_ajax.php',
								type: 'post',
