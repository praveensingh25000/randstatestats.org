...
if(!isset($_SESSION['crimes_us_asr']) || (isset($_SESSION['crimes_us_asr']) && !isset($_SESSION['crimes_us_asr']['dbid']))){
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: crimes_us_asr.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
...
$sql = "select distinct ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area in (".$area.") and ".$tablesname.".Cat1 in (".$category.") and ".$tablesname.".Category In (".$cats.") ".$joinStr." limit 4000";
