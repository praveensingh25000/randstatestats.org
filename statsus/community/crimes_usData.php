...
if(!isset($_SESSION['crimes_us']) || (isset($_SESSION['crimes_us']) && !isset($_SESSION['crimes_us']['dbid']))){
	...
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: crimes_us.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
	header('location: crimes_us.php');
	exit;
	}
}
...
$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area in (".$area.") and ".$tablesname.".Cat1 in (".$category.") and ".$tablesname.".Category In (".$cats.") ".$joinStr." limit 4000";
