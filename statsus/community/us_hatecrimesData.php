...
if(!isset($_SESSION['us_hatecrimes']) || (isset($_SESSION['us_hatecrimes']) && !isset($_SESSION['us_hatecrimes']['dbid']))){
	...
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: us_hatecrimes.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
...
if(!empty($searchedfields['cat1'])) {
	...
	$cat1Str = substr($catnameAll, 0, -1);
}

$CategoryStr = $CategoryAll = '';
...
$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area in (".$usstates.") and ".$tablesname.".Cat1 in (".$cat1Str.") and ".$tablesname.".Category In (".$CategoryStr.") ".$joinStr." order by ".$tablesname.".Category, ".$tablesname.".Area limit 4000";
