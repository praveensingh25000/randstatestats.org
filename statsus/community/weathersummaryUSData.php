...
if(!isset($_SESSION['weather']) || (isset($_SESSION['weather']) && !isset($_SESSION['weather']['dbid']))){
	...
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: weather.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
...
$states = $searchedfields['us_states'];
...
if(isset($statesArray)) {
	...
	$us_statesStr = substr($us_statesStr, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".State in (".$us_statesStr.")";
}

if(isset($searchedfields['area'])){
	$area = '';
...
if(isset($searchedfields['Cat1'])){
	$cat1 = '';
	foreach($searchedfields['Cat1'] as $keyArea => $catcode){
		$cat1 .= "'".$catcode."',";
	}
