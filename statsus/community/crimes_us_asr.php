...
#live Site URL For This Page: http://statestats.rand.org/stats/community/crimes_us_asr.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: crimes_us_asrData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: crimes_us_asrData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="crimes_us_asr">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_state_list_agencies_usr'])){ echo $lang['lbl_select_state_list_agencies_usr']; } ?></span>&nbsp;&nbsp;
						<?php if(isset($lang['lbl_states_crimes_usr'])){ echo $lang['lbl_states_crimes_usr']; } ?>
					</p>
					<div class="table-div">
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/community/crimes_us_asr_ajax.php',
								type: 'post',
