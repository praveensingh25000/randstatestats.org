...
#live Site URL For This Page: http://ny.rand.org/stats/community/weather.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: weathersummaryUSData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: weathersummaryUSData.php');
		exit;
		}
	}
}
...
$column = "statename";
$countiesResult = $admin->getDistinctColumnValuesUniversal($tablesnameStates , $column, 'statecode');
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/community/weathersummaryUSAjax.php',
								type: 'post',
								data: 'dbid=<?php echo $dbid; ?>&countries='+states,
