...
if(!isset($_SESSION['wateruseUSdet']) || (isset($_SESSION['wateruseUSdet']) && !isset($_SESSION['wateruseUSdet']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: wateruseUSdet.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: wateruseUSdet.php');
		exit;
	}
}

$searchedfields = $_SESSION['wateruseUSdet'];

$tablesname				= "wateruse";
$tablesnamecats			= "wateruse_cats";
