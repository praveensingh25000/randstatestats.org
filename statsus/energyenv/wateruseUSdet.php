...
* live Site: http://statestats.rand.org/stats/economics/wateruseUSdet.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: wateruseUSdetData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: wateruseUSdetData.php');
		exit;
		}
	}
}
								...
								onAdd: function (item) {
										var area      = jQuery('#us_states').val();
											...
											jQuery.ajax({								
												...
												url: URL_SITE+'/statsus/energyenv/wateruseUSdetAjax.php?dbid=<?php echo $dbid; ?>',
