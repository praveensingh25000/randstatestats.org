...
* live Site URL For This Page: http://statestats.rand.org/stats/energyenv/airquality_us.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: airquality_usData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: airquality_usData.php');
		exit;
		}
	}
}
...
$allStates = substr($allStates, 0, -1);
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="airquality_us">
				<div class="form-div">
					<p>
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/energyenv/airquality_us_ajax.php',
								type: 'post',
