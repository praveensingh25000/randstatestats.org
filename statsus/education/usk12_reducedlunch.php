...
if(!empty($_POST)){
	...
	$_SESSION['usk12_reducedlunch'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: usk12_reducedlunchData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: usk12_reducedlunchData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="usk12_reducedlunch">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_states_school_usk_reduced_lunch'])){ echo $lang['lbl_states_school_usk_reduced_lunch']; } ?> </span>&nbsp;&nbsp;
					</p>
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_states_usk_reduced_lunch'])){ echo $lang['lbl_states_usk_reduced_lunch']; } ?> </span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_school_usk_reduced_lunch'])){ echo $lang['lbl_school_usk_reduced_lunch']; } ?></span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
						<select size="4" name="sector" id="sector" class="required">
						<?php foreach($allSectors as $keySector => $detailSector){ 
							if(trim($detailSector['areatype']) != '' && trim($detailSector['areatype']) != 'District'){ ?>
				...
				$(document).ready(function() {	
					...
					$('#sector').change(function(){
						...
						if(states!='' && states != "NULL"){
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/education/usk12_reducedlunch_ajax.php',
								type: 'post',
