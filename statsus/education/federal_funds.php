...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
			header('location: federal_fundsData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: federal_fundsData.php');
		exit;
		}
	}
}
...
$level_res=$admin->getDistinctColumnValuesUniversal($tablesname,$column='level', $columns = "",$limit = "");
$levelArray = $dbDatabase->getAll($level_res);
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="federal_funds">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_please_select_level'])){ echo $lang['lbl_please_select_level']; } ?>
					   &nbsp;&nbsp;&nbsp;<!-- <input id="search_criteria_duplicates_all" type="checkbox" value="Y" name="allow_all" /> All --></span>

					   <script type="text/javascript">
						$(document).ready(function() {							
							jQuery("#search_criteria_duplicates_all").click(function(){					
								if(jQuery(this).is(":checked")){
									jQuery(".search_level_disable").attr('disabled', 'true');			
								} else {
									jQuery(".search_level_disable").removeAttr('disabled');
								}
							});
						});
						</script>
					...
					<div class="table-div">
						<?php if(!empty($levelArray)) {								
							$levelArrayDetail=array_chunk($levelArray,2,true);
							?>
							<table width="100%">								
								<?php foreach($levelArrayDetail as $keyAll => $valueAll) { ?>
									<tr>
									<?php foreach($valueAll as $key => $value) { ?>
										<td align="left">
											<input id="level_<?php echo $key;?>" type="radio" class="search_level_disable required" name="level[]" value="<?php echo $value['level'];?>" />&nbsp;&nbsp;&nbsp;<?php echo $value['level']?>
										</td>
										<td align="left">&nbsp;</td>
										<td align="left">&nbsp;</td>
										<td align="left">&nbsp;</td>
										...
										$(document).ready(function() {	
										   ...
										   $('#level_<?php echo $key;?>').click(function(){
												var level = jQuery('#level_<?php echo $key;?>').val();		
												...
												if(level!='' && level != null){
													...
													jQuery.ajax({
														...
														type: 'post',
														data: 'dbid=<?php echo $dbid; ?>&level='+level,
														success: function(dataresult){
															...
															if(obj.error == "0"){
																...
																if(!jQuery("#citiesDataLoadprogram").hasClass("programclass")){
																	jQuery('#citiesDataLoad').after('<div class="programclass pB10" id="citiesDataLoadprogram"></div>');
																}
