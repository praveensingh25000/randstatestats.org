...
if(isset($_POST['getresults'])){
	...
	$_SESSION['uscollegeenroll'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: uscollegeenrollData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: uscollegeenrollData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_states_school_enroll'])){ echo $lang['lbl_states_school_enroll']; } ?> </span>&nbsp;&nbsp;
					</p>
				...
				<?php
				$resultSector = $admin->getDistinctColumnValuesUniversal($tablesnamearea , 'sector');
				...
				<div class="form-div">
					...
					<div class="table-div">
						...
						<?php foreach($allSectors as $keySector => $detailSector){ 
							if(trim($detailSector['sector']) != '' && (trim($detailSector['sector']) != 'Central, System or Corporate Offices' && trim($detailSector['sector']) != 'sector not known')){ ?>
						<option value="<?php echo $detailSector['sector']; ?>"><?php echo $detailSector['sector']; ?></option>
						<?php } }?>
						</select>
					</div>
				  </div>
				...
				$(document).ready(function() {	
					...
					$('#sector').change(function(){
						...
						if(states!='' && states != "NULL"){
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/education/uscollegeenrollAjax.php',
								type: 'post',
							...
							alert("<?php if(isset($lang['lbl_states_school_enroll'])){ echo $lang['lbl_states_school_enroll']; } ?>");
						}
					});
