...
if(!isset($_SESSION['federal_funds']) || (isset($_SESSION['federal_funds']) && !isset($_SESSION['federal_funds']['dbid']))) {
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: federal_funds.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: federal_funds.php');
		exit;
	}
}

$searchedfields = $_SESSION['federal_funds'];
...
$levelAll = $levelStr = $departmentAll = $departmentStr = $programStr = $programAll = '';

$columnstobefetched = substr($columnstobefetched, 0, -1);

if(!empty($searchedfields['level'])){
	
	foreach($searchedfields['level'] as $level){
		$levelAll.="'".$level."'".',';
	}
	$levelStr = substr($levelAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".level in (".$levelStr.") ";
}

if(!empty($searchedfields['department'])){
	
	foreach($searchedfields['department'] as $department){
		$departmentAll.="'".$department."'".',';
	}
	$departmentStr = substr($departmentAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".department in (".$departmentStr.") ";
}

if(!empty($searchedfields['program'])){

	foreach($searchedfields['program'] as $program){
		$programAll.="'".$program."'".',';
	}
	$programStr = substr($programAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".program in (".$programStr.") ";
}
		...
		if(count($searchedData)>0) {
			...
			<div class="search-table-data" style="<?php if((isset($_GET['graph']) && $_GET['graph'] != 'gridview')) { ?> display:none; <?php } ?>">
				...
				<thead>
						...
						$i=0;
						$arrayColumns = $footNote = array();
						
						foreach($searchedData as $keyData => $rowData){ 	
						
						$strGraph = $footnotesIds = '';
						
						if(isset($rowData['Footnote']) && $rowData['Footnote']!='' && $rowData['Footnote']!='NA'){
							
							$footnotesIds .= $rowData['Footnote'];							
							$footnotesExplodedIds = explode(",",str_replace(" ","",$footnotesIds));
