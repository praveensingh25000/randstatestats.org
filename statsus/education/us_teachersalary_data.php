...
if(!isset($_SESSION['us_teachersalary']) || (isset($_SESSION['us_teachersalary']) && !isset($_SESSION['us_teachersalary']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: us_teachersalary.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: us_teachersalary.php');
		exit;
	}
}

$searchedfields = $_SESSION['us_teachersalary'];
...
if(isset($searchedfields['category'])){
	$cat1 = '';
	foreach($searchedfields['category'] as $keyc => $catst){
		$cat1 .= "'".$catst."',";
	}
	$cat1 = substr($cat1, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".Category in (".$cat1.") ";
}
