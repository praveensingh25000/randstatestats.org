...
if(!isset($_SESSION['uscollegeenroll']) || (isset($_SESSION['uscollegeenroll']) && !isset($_SESSION['uscollegeenroll']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: uscollegeenroll.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: uscollegeenroll.php');
		exit;
	}
}

$searchedfields = $_SESSION['uscollegeenroll'];
...
if(isset($searchedfields['Cat1'])){
	$cat1 = '';
	foreach($searchedfields['Cat1'] as $keyc => $catst){
		$cat1 .= "'".$catst."',";
	}
...
$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area in (".$area.")  and ".$tablesnameatcat.".catcode In (".$catcode.") ".$joinStr." limit 4000";
