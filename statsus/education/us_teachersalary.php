...
if(isset($_POST['getresults'])){
	...
	$_SESSION['us_teachersalary'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: us_teachersalary_data.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: us_teachersalary_data.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_enter_state'])){ echo $lang['lbl_enter_state']; } ?> </span>&nbsp;&nbsp;
					</p>
				...
				$(document).ready(function() {	
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { tokenLimit: 1, preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/education/us_teachersalary_ajax.php',
								type: 'post',
