...
if(!isset($_SESSION['usk12dropout']) || (isset($_SESSION['usk12dropout']) && !isset($_SESSION['usk12dropout']['dbid']))){
	...
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: usk12dropout.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
...
if(isset($searchedfields['areacode']) && count($searchedfields['areacode'])>0){
	$area = explode(';', $searchedfields['areacode']);
	$areastr = '';
	foreach($area  as $arekey => $area){
		$areastr .= "'".$area."',";
	}
