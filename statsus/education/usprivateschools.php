...
* @live Site URL For This Page: http://statestats.rand.org/statsus/education/usprivateschools.html
********************************************/
...
$tablesnamearea = "usprivateschools_areas";
...
if(isset($_POST['getresults'])){
	...
	$_SESSION['usprivateschools'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: usprivateschoolsData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: usprivateschoolsData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="usprivateschools">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_state_type_school'])){ echo $lang['lbl_select_state_type_school']; } ?> </span>&nbsp;&nbsp;
					</p>
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_states_us_private_school'])){ echo $lang['lbl_states_us_private_school']; } ?> </span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
				...
				<?php
				$resultSector = $admin->getDistinctColumnValuesUniversal($tablesname , 'SchoolType');
				$allSectors = $dbDatabase->getAll($resultSector );
				?>
				<!-- form -->
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_school_type'])){ echo $lang['lbl_school_type']; } ?></span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
						<select size="4" name="sector" id="sector" class="required">
						<?php foreach($allSectors as $keySector => $detailSector){ ?>
						<option value="<?php echo $detailSector['SchoolType']; ?>"><?php echo $detailSector['SchoolType']; ?></option>
						<?php } ?>
						</select>
					</div>
				  </div>
				...
				$(document).ready(function() {	
					...
					$('#sector').change(function(){
						...
						if(states!='' && states != "NULL"){
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/education/usprivateschoolsAjax.php',
								type: 'post',
