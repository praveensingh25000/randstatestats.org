...
* @live Site URL For This Page: http://statestats.rand.org/stats/education/usk12dropout.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: usk12dropoutData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: usk12dropoutData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="usk12dropout">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_state_dropout'])){ echo $lang['lbl_select_state_dropout']; } ?></span>&nbsp;&nbsp;
						<?php if(isset($lang['lbl_states_dropout'])){ echo $lang['lbl_states_dropout']; } ?>
					</p>
					<div class="table-div">
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { tokenLimit: 1, preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/education/usk12dropoutAjax.php',
								type: 'post',
