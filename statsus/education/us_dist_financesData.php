...
if(!isset($_SESSION['us_dist_finances']) || (isset($_SESSION['us_dist_finances']) && !isset($_SESSION['us_dist_finances']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: us_dist_finances.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: us_dist_finances.php');
		exit;
	}
}

$searchedfields = $_SESSION['us_dist_finances'];
...
$areastr = '';
if(count($searchedfields['areacode'])>0){
	foreach($searchedfields['areacode'] as $keystate => $areas){
