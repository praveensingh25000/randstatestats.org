...
* @live Site URL For This Page: http://statestats.rand.org/stats/education/usk12_reducedlunch.html
********************************************/
...
if(!isset($_SESSION['usk12_reducedlunch']) || (isset($_SESSION['usk12_reducedlunch']) && !isset($_SESSION['usk12_reducedlunch']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: usk12_reducedlunch.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: usk12_reducedlunch.php');
		exit;
	}
}

$searchedfields = $_SESSION['usk12_reducedlunch'];
...
if(!empty($searchedfields['areacode'])){
	$area = implode(',', $searchedfields['areacode']);
	$fieldsStrWhere .= " and ".$tablesname.".Area in (".$area.") ";
}


$catcode = implode(',', $searchedfields['catcode']);

$sector = $searchedfields['sector'];

$us_states = $searchedfields['us_states'];
$states = explode(';', $us_states);
...
if(!empty($states)){
	...
	$stateStr = substr($stateStr, 0, -1);
	$fieldsStrWhere .= " and ".$tablesnamearea.".areast in (".$stateStr.") ";

}

$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere."   and ".$tablesnameatcat.".catcode In (".$catcode.") and usk12enroll_areas.areatype = '".$sector."' ".$joinStr." limit 4000";
