...
* live Site URL For This Page: statsus/avgwageUS.php
********************************************/
...
if(isset($_REQUEST['Sector']) && $_REQUEST['Sector']!='' && isset($_REQUEST['Cat2']) && count($_POST['Cat2'])>0 && isset($_POST['us_states']) && $_POST['us_states']!='') {
	...
	foreach($_POST['Cat2'] as $key => $cat2){
		...
		$sql = "SELECT DISTINCT(avc.catcode), avc.catname FROM `avgwage1US` as av left join avgwage1US_cats as avc on av.Category = avc.catcode WHERE Cat1 = '".$_POST['Sector']."' and Cat2 = '".$cat2."' order by avc.catname";
