...
if(isset($_POST['getresults'])){
	...
	$_SESSION['residential_construction'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: residentialconstructionData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: residentialconstructionData.php');
			exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<input type="hidden" name="session_setter" value="residential_construction">
				<div class="form-div">
					<p>
					   <span class="choose">Please enter a state.</span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
						<input type="text" id="search_criteria_duplicates" name="states"/>
					</div>
				</div>

				<div class="pB10" id="countiesDataLoad">
				</div>
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJsonState; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/economics/residentialconstruction_ajax.php',
								type: 'post',
