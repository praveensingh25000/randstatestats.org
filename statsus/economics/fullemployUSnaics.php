<?php
/******************************************
* @Modified on Modified on April 10, 2013
* @Package: Rand
...
if(isset($_POST['getresults'])){
	...
	$_SESSION['fullemployUSnaics'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: fullemployUSnaicsData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: fullemployUSnaicsData.php');
			exit;
		}
	}
}
								...
								onAdd: function (item) {
										var area      = jQuery('#us_states').val();
											...
											jQuery.ajax({								
												...
												url: URL_SITE+'/statsus/economics/fullemployUSnaicsAjax.php?dbid=<?php echo $dbid; ?>',
