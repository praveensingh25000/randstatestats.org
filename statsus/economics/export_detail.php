<?php
/******************************************
* @Modified on Modified on May 13, 2013
* @Package: Rand
* @Developer: Baljinder Singh
* @URL : http://50.62.142.193/
* live Site: http://statestats.rand.org/stats/economics/bankruptUSdet.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: export_detailData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: export_detailData.php');
			exit;
		}
	}
}
...
$popcontent = '';
foreach($comAgg as $key => $value){
	$stateToNamefiltervaluesJsonArray[] = array('id' => $key, 'name' => $value);
	$popcontent .= mysql_real_escape_string($value.'<br/>');
}
	...
	<section id="inner-content">
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_enter_commodities'])){ echo $lang['lbl_enter_commodities']; } ?></span>&nbsp;&nbsp;&nbsp;<a id="viewall" href="javascript:;" onclick="javascript: return popup_window('<?php echo $popcontent; ?>');">See a list</a>
					</p>
					<div class="table-div">	

						<p><input id="us_states" class="required" type="input"  name="comgroups"/></p>
								...
								onAdd: function (item) {
										var area      = jQuery('#us_states').val();
											...
											jQuery.ajax({								
												...
												url: URL_SITE+'/statsus/economics/export_detail_ajax.php?dbid=<?php echo $dbid; ?>',
