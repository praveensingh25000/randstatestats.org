...
if(!isset($_SESSION['airline_operations']) || (isset($_SESSION['airline_operations']) && !isset($_SESSION['airline_operations']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: airline_operations.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: airline_operations.php');
		exit;
	}
}

$searchedfields = $_SESSION['airline_operations'];
...
if(isset($searchedfields['airlines'])){
	$statesAr = explode(";", $searchedfields['airlines']);
	
	$airlinecodes = "";

	foreach($statesAr as $ak => $airlinecode){
		$airlinecodes .= "'".$airlinecode."', ";
	}

	$airlinecodes = substr($airlinecodes, 0, -2);

	$fieldsStrWhere .= " and ".$tablesname.".airline_code in (".$airlinecodes.") ";
}
...
$sql = "select ".$columnstobefetched." from ".$tablesname.", ".$tablesnamearea." where 1 ".$fieldsStrWhere."  ".$joinStr." order by ".$tablesname.".year limit 4000";
