...
if(!isset($_SESSION['airport_us']) || (isset($_SESSION['airport_us']) && !isset($_SESSION['airport_us']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: airport_us.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: airport_us.php');
		exit;
	}
}

$searchedfields = $_SESSION['airport_us'];
