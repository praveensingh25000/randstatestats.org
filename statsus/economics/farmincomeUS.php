...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: farmincomeUSData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: farmincomeUSData.php');
			exit;
		}
	}
}
								...
								onAdd: function (item) {
										var area      = jQuery('#us_states').val();
											...
											jQuery.ajax({								
												...
												url: URL_SITE+'/statsus/economics/farmincomeUSAjax.php?dbid=<?php echo $dbid; ?>',
