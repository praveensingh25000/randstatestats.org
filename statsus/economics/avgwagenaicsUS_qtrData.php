...
if(!isset($_SESSION['avgwagenaicsUSqtr']) || (isset($_SESSION['avgwagenaicsUSqtr']) && !isset($_SESSION['avgwagenaicsUSqtr']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: avgwagenaicsUS_qtr.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: avgwagenaicsUS_qtr.php');
		exit;
	}
}

$searchedfields = $_SESSION['avgwagenaicsUSqtr'];
...
if(!empty($searchedfields['Cat3'])){
	...
	$fieldsStrWhere.=" and ".$tablesname.".Cat3 in (".$cat3Str.") ";
}
...
<section id="container">
 ...
 <div class="main-cell">	
		...
		?>
			...
			<?php 
			$nograph= '1';	
			include($DOC_ROOT."/subPageLinks.php"); ?>
			<!-- /PAGE LINKS -->
