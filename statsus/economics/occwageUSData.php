...
if(!isset($_SESSION['occwageUS']) || (isset($_SESSION['occwageUS']) && !isset($_SESSION['occwageUS']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: occwageUS.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: occwageUS.php');
		exit;
	}
}

$searchedfields = $_SESSION['occwageUS'];
...
$sql = "select ".$columnstobefetched." from occwage2, occwage2_areas, occwage2_cats where 1 ".$fieldsStrWhere." and ".$tablesname.".Category IN (".$categoryStr.") and ".$tablesname.".Cat1 IN (".$cat1Str.")  ".$joinStr." limit 4000";
