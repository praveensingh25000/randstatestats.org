...
if(!isset($_SESSION['median_income']) || (isset($_SESSION['median_income']) && !isset($_SESSION['median_income']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {

		header('location: median_income.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: median_income.php');
		exit;
	}
}

$searchedfields = $_SESSION['median_income'];
...
$cats = implode(',', $searchedfields['catcode']);

$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and medianinc_areas.areacode in (".$area.") and medianinc_cats.catcode In (".$cats.") ".$joinStr." limit 4000";
