...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: econprofUSData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: econprofUSData.php');
			exit;
		}
	}
}
								...
								onAdd: function (item) {
										var area      = jQuery('#us_states').val();
											...
											jQuery.ajax({								
												...
												url: URL_SITE+'/statsus/economics/econprofUSAjax.php?dbid=<?php echo $dbid; ?>',
