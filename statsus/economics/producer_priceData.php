...
$joinStr = $tablesStr = $fieldsStrWhere = $columnstobefetched = $catname = "";

if(!isset($_SESSION['producer_price']) || (isset($_SESSION['producer_price']) && !isset($_SESSION['producer_price']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: producer_price.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: producer_price.php');
		exit;
	}
}

$searchedfields = $_SESSION['producer_price'];
...
$industryAll = $industryStr = '';
if(isset($searchedfields['industry'])){
	$industryArray = explode(';',$searchedfields['industry']);
	foreach($industryArray as $keyArea => $industry){
		$industryAll .= "'".$industry."',";
	}
	$industryStr = substr($industryAll, 0, -1);
	$fieldsStrWhere.= " and ".$tablesname.".industry IN (".$industryStr.") ";
}

$product = $productStr = '';
if(isset($searchedfields['product'])){
	foreach($searchedfields['product'] as $keyArea => $categoryname){
		$product .= "'".$categoryname."',";
	}
	$productStr = substr($product, 0, -1);
	$fieldsStrWhere.= " and ".$tablesname.".product IN (".$productStr.") ";
}
 ...
 <div class="main-cell">	
		...
		if((mysql_num_rows($searchedDataResult))>0){
			...
			<table id="" class="data-table">
				...
				if(isset($columns['quatersascolumns']) && isset($columns['yearsasrows']) && isset($timeIntervalSettings) && ($timeIntervalSettings['time_format'] == 'SQ-SY-EQ-EY' || $timeIntervalSettings['time_format'] == 'SY-EY')){		// When data is as year q1, q2, q3, q4
					...
					while($rowData = mysql_fetch_assoc($searchedDataResult)){ 
						...
						if( (array_key_exists('Industry', $rowData) || array_key_exists('industry', $rowData)) && (array_key_exists('Product', $rowData) || array_key_exists('product', $rowData)) && (array_key_exists('Year', $rowData) || array_key_exists('year', $rowData))) {	
							
							$area = (array_key_exists('Industry', $rowData))?$rowData['Industry']:$rowData['industry'];
							$category = (array_key_exists('Product', $rowData))?$rowData['Product']:$rowData['product'];
							$year = (array_key_exists('Year', $rowData))?$rowData['Year']:$rowData['year'];
			...
			<p>&nbsp;</p>
				...
				<strong><?php echo $NoteId;?>:&nbsp;&nbsp;</strong><?php echo $NoteDescr;
				echo '<br/><br/>';}?>
			</p>
			<?php } ?>
			<!-- Show Notes -->
