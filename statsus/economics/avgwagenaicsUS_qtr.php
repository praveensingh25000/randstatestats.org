...
if(isset($_POST['getresults'])){
	...
	$_SESSION['avgwagenaicsUSqtr'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: avgwagenaicsUS_qtrData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: avgwagenaicsUS_qtrData.php');
			exit;
		}
	}
}
								...
								onAdd: function (item) {
										...
										if(area == ""){
											...
											jQuery.ajax({								
												...
												url: URL_SITE+'/statsus/economics/avgwagenaicsUS_qtr_ajax.php?dbid=<?php echo $dbid; ?>&Sector='+Sector+"&owner="+owner,
