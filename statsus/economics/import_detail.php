...
* live Site: http://statestats.rand.org/stats/economics/import_detail.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: import_detailData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: import_detailData.php');
			exit;
		}
	}
}
								...
								onAdd: function (item) {
										var area      = jQuery('#us_states').val();
											...
											jQuery.ajax({								
												...
												url: URL_SITE+'/statsus/economics/import_detail_ajax.php?dbid=<?php echo $dbid; ?>',
