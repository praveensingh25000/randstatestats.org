...
if(!isset($_SESSION['avgwageUS']) || (isset($_SESSION['avgwageUS']) && !isset($_SESSION['avgwageUS']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: avgwageUS.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: avgwageUS.php');
		exit;
	}
}

$searchedfields = $_SESSION['avgwageUS'];
