...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: bankruptUSdetData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: bankruptUSdetData.php');
			exit;
		}
	}
}
	...
	<section id="inner-content">
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				<input type="hidden" name="session_setter" value="bankruptUSdet">
				<!-- form -->
				<div class="form-div">
					<p>
								...
								onAdd: function (item) {
										var area      = jQuery('#us_states').val();
											...
											jQuery.ajax({								
												...
												url: URL_SITE+'/statsus/economics/bankruptUSdetAjax.php?dbid=<?php echo $dbid; ?>',
