...
if(!isset($_SESSION['usconstpldata']) || (isset($_SESSION['usconstpldata']) && !isset($_SESSION['usconstpldata']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: usconstPL.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: usconstPL.php');
		exit;
	}
}

$searchedfields = $_SESSION['usconstpldata'];
...
if(isset($searchedfields['catcode'])){
	$cats = implode(',', $searchedfields['catcode']);
	$fieldsStrWhere .= 	" and ".$tablesname.".Category in (".$cats.") ";
}
