...
if(!isset($_SESSION['bankruptUSdet']) || (isset($_SESSION['bankruptUSdet']) && !isset($_SESSION['bankruptUSdet']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: bankruptUSdet.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: bankruptUSdet.php');
		exit;
	}
}

$searchedfields = $_SESSION['bankruptUSdet'];
