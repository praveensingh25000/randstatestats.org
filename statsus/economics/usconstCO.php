...
* live Site URL For This Page:		 http://statestats.rand.org/stats/economics/usconstCO.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: usconstCOData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: usconstCOData.php');
		exit;
		}
	}
}
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { tokenLimit: 1, preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/economics/usconstCOAjax.php',
								type: 'post',
