...
* live Site URL For This Page:		 http://statestats.rand.org/stats/economics/usconstPL.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: usconstPLData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: usconstPLData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_choose_construction_area'])){ echo $lang['lbl_choose_construction_area']; } ?></span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
				...
				<div class="pB10" id="citiesDataLoadCounties"></div>
				<div class="pB10" id="citiesDataLoadAreas"></div>
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { tokenLimit: 1, preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/economics/usconstPLAjax.php',
								type: 'post',
								...
								success: function(dataresult){
									...
									if(obj.error == "0"){
										jQuery('#citiesDataLoadCounties').html(obj.data);
										jQuery('#timePeriod').show();
										jQuery('#submitButtons').show();
									} else {
										jQuery('#citiesDataLoadCounties').html('');
