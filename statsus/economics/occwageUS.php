...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: occwageUSData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: occwageUSData.php');
		exit;
		}
	}
}
...
/*$area_res   = $admin->getTableDataUniversal($tablesnamecountyarea);
$areaDetail = $dbDatabase->getAll($area_res);

foreach($areaDetail as $key => $value){
$areavaluesJsonArray[] = array('id' => $value['statecode'], 'name' => $value['statename']);
}*/

$area_res   = $admin->getTableDataUniversal($tablesnamearea);
$areaDetail = $dbDatabase->getAll($area_res);

$areavaluesJsonArray = array();
foreach($areaDetail as $key => $value){
$areavaluesJsonArray[] = array('id' => $value['areacode'], 'name' => $value['areaname']);
}

$areavaluesJson = json_encode($areavaluesJsonArray);
?>
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
					...
					<!-- form -->
					<input type="hidden" name="session_setter" value="occwageUS">

					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_Choose_four_category'])){ echo $lang['lbl_Choose_four_category']; } ?> </span>
						</p>
						<div class="table-div">
							<div class="wdthpercent50 left">
							<input type="text" id="search_criteria_duplicates" name="category"/>
							</div>
							<div class="pL20 left">&nbsp;&nbsp;<a onclick="javascript: return pop_window_url('<?php echo URL_SITE."/allValues.php?table=".$tablesnamepatcat."&column=catename"; ?>');" href="javascript:;">See a list</a></div>
							<div class="clear"></div>
						</div>
					</div>					
					...
					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_Choose_four_states'])){ echo $lang['lbl_Choose_four_states']; } ?> </span>
						</p>
						<div class="table-div">	
							
							<div class="wdthpercent50 left">
								<input type="text" id="search_criteria_duplicates_states" name="us_states"/>
							</div>
							<div class="pL20 left">
								&nbsp;&nbsp;<a onclick="javascript: return pop_window_url('<?php echo URL_SITE."/allValues.php?table=".$tablesnamearea."&column=areaname"; ?>');" href="javascript:;">See a list</a>
							</div>
							<div class="clear"></div>
							<!-- <?php foreach($areavaluesJsonArray as $key => $areavalues){ ?>
								<p><input class="required" type="checkbox" value="<?=$areavalues['id']?>" name="us_states[]"/>&nbsp;<?=$areavalues['name']?></p>
							<?php } ?> -->								
						</div>
					</div>
					<!-- /form -->					
					...
					$(document).ready(function() {							
						$("#search_criteria_duplicates_states").tokenInput(<?php echo $areavaluesJson; ?>, {		
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								var category = jQuery('#search_criteria_duplicates').val();
								loader_show();
								jQuery('#timePeriod').hide();
								jQuery('#submitButtons').hide();
								jQuery.ajax({
									url: URL_SITE+'/statsus/economics/occwageUSAjax.php',
									type: 'post',
