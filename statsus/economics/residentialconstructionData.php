...
if(!isset($_SESSION['residential_construction']) || (isset($_SESSION['residential_construction']) && !isset($_SESSION['residential_construction']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: residentialconstruction.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: residentialconstruction.php');
		exit;
	}
}

$searchedfields = $_SESSION['residential_construction'];
...
if(isset($searchedfields['counties'])){

	$counties = explode(";", $searchedfields['counties']);
	
	$countiesIn = implode(",", $counties);

	$fieldsStrWhere .= " and ".$tablesname.".county_code in (".$countiesIn.") ";
}

if(isset($searchedfields['cities'])){
	
	$areacode = implode(",", $searchedfields['cities']);

	$fieldsStrWhere .= " and ".$tablesname.".area_code in (".$areacode.") ";

}

if(isset($searchedfields['zips'])){
	if($searchedfields['zips']!=""){
		$zipsArr = explode(";", $searchedfields['zips']);

		$zips = implode(",", $zipsArr);

		$fieldsStrWhere .= " and ".$tablesname.".zip in (".$zips.") ";
	}
}
