...
if(!isset($_SESSION['usconstdata']) || (isset($_SESSION['usconstdata']) && !isset($_SESSION['usconstdata']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: usconstCO.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: usconstCO.php');
		exit;
	}
}

$searchedfields = $_SESSION['usconstdata'];
