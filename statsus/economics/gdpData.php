...
if(!isset($_SESSION['gdp']) || (isset($_SESSION['gdp']) && !isset($_SESSION['gdp']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: gdp.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: gdp.php');
		exit;
	}
}

$searchedfields = $_SESSION['gdp'];
...
if(!empty($searchedfields['industry_name'])){
	$industrynameStr = $industrynameAll ='';
	$industrynameArray = explode(';',$searchedfields['industry_name']);
	foreach($industrynameArray as $industryname){
		$industrynameAll.="'".$industryname."'".',';
	}
	$industrynameStr = substr($industrynameAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".industry_name in (".$industrynameStr.") ";
}

if(!empty($searchedfields['States'])){
	$StatesStr = $StatesStrAll ='';
	$StatesArray = $searchedfields['States'];
	foreach($StatesArray as $States){
		$StatesStrAll.="'".$States."'".',';
	}
	$StatesStr = substr($StatesStrAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".States in (".$StatesStr.") ";
}
