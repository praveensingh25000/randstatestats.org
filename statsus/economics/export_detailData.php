...
if(!isset($_SESSION['exportdetail']) || (isset($_SESSION['exportdetail']) && !isset($_SESSION['exportdetail']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: export_detail.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: export_detail.php');
		exit;
	}
}

$searchedfields = $_SESSION['exportdetail'];

//echo "<pre>";print_r($searchedfields);echo "</pre>"; die;


$tablesname				= "exportdet_qtr_";
$tablesnamecat			= "export_cats";
$tablesnamecomm			= "export_commodities";
