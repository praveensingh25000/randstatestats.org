...
if(!isset($_SESSION['airport_usNew']) || (isset($_SESSION['airport_usNew']) && !isset($_SESSION['airport_usNew']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: airport_usNew.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: airport_usNew.php');
		exit;
	}
}

$searchedfields = $_SESSION['airport_usNew'];
			...
			for($i=$searchedfields['squater'];$i<=$searchedfields['equater'];$i++){
				...
				if(in_array($embedQ.''.$col, $tableColumnsArray) || in_array(strtoupper($embedQ).''.$col, $tableColumnsArray)){
					...
					if($embedM!=''){
						...
						$columnstobefetched .= 'sum('.$table.'.'.$embedQ.''.$col.") as '".$quaters[$i]."',";
					} else {
						$quatersArray[] = $embedQ.''.$col;
						$columnstobefetched .= 'sum('.$table.'.'.$embedQ.''.$col.") as '".$embedQ.''.$col."',";
					}

					$totalColumn .= 'sum('.$table.'.'.$embedQ.''.$col.") + ";
				}
			}

			$totalColumn = substr($totalColumn, 0, -2);
		}
...
$area = '';
if(isset($searchedfields['areacode'])){
	foreach($searchedfields['areacode'] as $keyArea => $areaCode){
		$area .= "'".$areaCode."',";
	}

	$area = substr($area, 0, -1);

	$fieldsStrWhere .= " and ".$tablesname.".origin_airport_id in (".$area.") ";
}

if(isset($searchedfields['us_states'])){
	$statesAr = explode(";", $searchedfields['us_states']);
	
	$states = implode(",", $statesAr);

	$fieldsStrWhere .= " and ".$tablesname.".origin_state_code in (".$states.") ";
}


$category = '';
if(isset($searchedfields['category'])){
	foreach($searchedfields['category'] as $keyArea => $categoryname){
		$category .= "'".$categoryname."',";
	}

	$category = substr($category, 0, -1);

	$fieldsStrWhere .= " and ".$tablesname.".Category in (".$category.") ";
}

if(isset($totalColumn)){
	$columnstobefetched .= ", (".$totalColumn.") as Total ";
}

$sql = "select ".$columnstobefetched." from ".$tablesname.", ".$tablesnamearea." where 1 ".$fieldsStrWhere."  ".$joinStr." GROUP BY ".$tablesname.".year, ".$tablesname.".origin_airport_id limit 4000";
...
<section id="container">
 ...
 <div class="main-cell">	
		...
		?>
				...
				if(isset($columns['quatersascolumns']) && isset($columns['yearsasrows']) && isset($timeIntervalSettings) && ($timeIntervalSettings['time_format'] == 'SQ-SY-EQ-EY' || $timeIntervalSettings['time_format'] == 'SY-EY')){		// When data is as year q1, q2, q3, q4
					...
					foreach($yearArray as $keyY => $year){
						foreach($quatersArray as $keyQ => $quater){
							$tempYears[] = $quater.'/'.substr($year, -4); 
					...
					foreach($searchedData as $keyData => $rowData){ 
						...
						}else if(array_key_exists('State', $rowData) && array_key_exists('Category', $rowData) && array_key_exists('Year', $rowData)){
							$columnsArrayData[$rowData['State']][$rowData['Category']][$rowData['Year']] = $rowData;
						}else if(array_key_exists('Airport', $rowData) && array_key_exists('Category', $rowData) && array_key_exists('Year', $rowData)){


							$rowData['Area'] = $rowData['Airport'];
							$columnsArrayData[$rowData['Airport']][$rowData['Category']][$rowData['Year']] = $rowData;
						}
			...
			<!-- GRAPH VIEW -->
			<?php 
			if(!empty($arrayColumns) && $strGraph!='') {
				include($DOC_ROOT."/graphInclude.php");
			} else {
				echo '<h4 class="txtcenter pT20 pB20">Sufficient data are not available for drawing a graph</h4>';
			}
			?>
			<!-- GRAPH VIEW -->
