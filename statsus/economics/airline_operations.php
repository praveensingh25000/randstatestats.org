...
if(isset($_POST['getresults'])){
	...
	$_SESSION['airline_operations'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: airline_operationsData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: airline_operationsData.php');
			exit;
		}
	}
}
...
$statetoname_res  = $admin->getTableDataUniversal($tablesnameairlines);
...
foreach($stateToName as $key => $value){
	if($value['name']!=NULL){
		$stateToNamefiltervaluesJsonArray[] = array('id' => $value['airline_code'], 'name' => $value['name']);
	}
}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<input type="hidden" name="session_setter" value="airline_operations">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_choose_airline'])){ echo $lang['lbl_choose_airline']; } ?></span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
						<input type="text" id="search_criteria_duplicates" name="airlines"/>
					</div>
				</div>
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJsonState; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/economics/airlines_operations_ajax.php',
								type: 'post',
								data: 'dbid=<?php echo $dbid; ?>&airlines='+states,
