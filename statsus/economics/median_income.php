...
$dbname = $dbsource = $description = $miscellaneous = $stateToNameContent = '';

$databaseCategories = $databaseRelatedDatabases = array();

if(isset($_POST['getresults'])){

	$_SESSION['median_income'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: median_incomeData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
			header('location: median_incomeData.php');
			exit;
		}
	}
}
...
foreach($stateToName as $key => $value){
  if(strtolower($value) != 'united states'){
	 $stateToNameContent .= '<p>'.$value."</p>";
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="median_income">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_choose_metropolitan_area'])){ echo $lang['lbl_choose_metropolitan_area']; } ?></span>&nbsp;&nbsp;&nbsp;<input id="search_criteria_metropolitan_all" type="checkbox" value="All" name="allow_all" /> All

					   <script type="text/javascript">
						$(document).ready(function() {							
							jQuery("#search_criteria_metropolitan_all").click(function(){					
								if(jQuery(this).is(":checked")){
									jQuery('#citiesDataLoad').html('');									
								    jQuery("#search_criteria_duplicates").tokenInput("toggleDisabled");
									var states = jQuery('#search_criteria_metropolitan_all').val();
									loader_show();
									jQuery('#timePeriod').hide();
									jQuery('#submitButtons').hide();
									jQuery.ajax({
										url: '<?php echo URL_SITE; ?>/statsus/economics/median_income_ajax.php',
										type: 'post',
					...
					<div class="table-div">
						...
						<div class="left pL10">
							&nbsp;&nbsp;<a id="viewall" href="javascript:;" onclick="javascript: return popup_window('<?php echo $stateToNameContent; ?>');">See a list</a>
						</div>
					</div>
				  </div>
				<!-- /form -->
