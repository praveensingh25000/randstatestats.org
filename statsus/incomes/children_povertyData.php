...
if(!isset($_SESSION['children_poverty']) || (isset($_SESSION['children_poverty']) && !isset($_SESSION['children_poverty']['dbid']))) {
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: children_poverty.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: children_poverty.php');
		exit;
	}
}

$searchedfields = $_SESSION['children_poverty'];
...
$StateStr = $StateAll = $areaStr = $categoryAll = $categoryStr = $CountyStr = $CountyAll = '';

$columnstobefetched = substr($columnstobefetched, 0, -1);

if(isset($searchedfields['State']) && $searchedfields['State']!='') {
	$StateArray = explode(';', $searchedfields['State']);
	foreach($StateArray as $Statekey => $State){
		$StateAll .= "'".$State."',";
	}
	$StateStr = substr($StateAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".State in (".$StateStr.") ";
}

if(!empty($searchedfields['County'])){
	foreach($searchedfields['County'] as $County){
		$CountyAll.="'".$County."'".',';
	}
	$CountyStr = substr($CountyAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".County in (".$CountyStr.") ";
}

if(isset($searchedfields['Category']) && $searchedfields['Category']!='') {
	$categoryArray = explode(';', $searchedfields['Category']);
	foreach($categoryArray as $keycategory => $category){
