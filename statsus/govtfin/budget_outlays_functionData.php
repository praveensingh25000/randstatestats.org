...
if(!isset($_SESSION['budget_outlays_function']) || (isset($_SESSION['budget_outlays_function']) && !isset($_SESSION['budget_outlays_function']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: budget_outlays_function.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: budget_outlays_function.php');
		exit;
	}
}

$searchedfields = $_SESSION['budget_outlays_function'];
...
$provisionAll = $provisionStr = $functionStrAll = $functionStr = '';

$functionArray = explode(';',$searchedfields['function']);
if(!empty($functionArray)){
	foreach($functionArray as $function){
		$functionStrAll.="'".$function."'".',';
	}
	$functionStr = substr($functionStrAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".function IN (".$functionStr.") ";
}

if(!empty($searchedfields['subfunction'])){
	
	foreach($searchedfields['subfunction'] as $provision){
		$provisionAll.="'".$provision."'".',';
	}
	$provisionStr = substr($provisionAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".subfunction IN (".$provisionStr.") ";
}
				...
				<table id="" class="data-table">
					...
					if(isset($columns['quatersascolumns']) && isset($columns['yearsasrows']) && isset($timeIntervalSettings) && ($timeIntervalSettings['time_format'] == 'SQ-SY-EQ-EY' || $timeIntervalSettings['time_format'] == 'SY-EY')){		// When data is as year q1, q2, q3, q4
						...
						foreach($searchedData as $keyData => $rowData){ 
							...
							} else if((array_key_exists('Function', $rowData) || array_key_exists('function', $rowData)) && (array_key_exists('Subfunction', $rowData) || array_key_exists('subfunction', $rowData)) && (array_key_exists('Year', $rowData) || array_key_exists('year', $rowData))){
								
								$origin = (array_key_exists('Function', $rowData))?$rowData['Function']:$rowData['function'];
								$category = (array_key_exists('Subfunction', $rowData))?$rowData['Subfunction']:$rowData['subfunction'];
