...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: cffus_detData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: cffus_detData.php');
		exit;
		}
	}
}
...
//categpry Detail
$allCategoryDetail_res  = $admin->getDistinctColumnValuesUniversal($tablesname , 'Category', $columnnamevalue='', $orderby = '');
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
					...
					$(document).ready(function() {							
						$('.on_click_all').click(function(){
							var states = jQuery('#search_criteria_duplicates').val();
							var category = '';
							$("input:checkbox[class=on_click_all]:checked").each(function()
							{
								category = category + ';' + $(this).val();
							});

							if(states == ''){
								alert("<?php if(isset($lang['lbl_select_more_areas_categories'])){ echo $lang['lbl_select_more_areas_categories']; } ?>");
								return false;
							}
							...
							jQuery.ajax({
								url: URL_SITE+'/statsus/govtfin/cffus_det_ajax.php',
								type: 'post',
								data: 'dbid=<?php echo $dbid; ?>&states='+states+'&category='+category,
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJsonState; ?>, {		
