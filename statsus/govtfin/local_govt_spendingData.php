...
if(!isset($_SESSION['local_govt_spending']) || (isset($_SESSION['local_govt_spending']) && !isset($_SESSION['local_govt_spending']['dbid']))){
	...
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: local_govt_spending.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: local_govt_spending.php');
		exit;
	}
}
...
if(!empty($searchedfields['areacode'])){
	$areasAll = $areaStr='';
	foreach($searchedfields['areacode'] as $states){
		$areasAll.="'".$states."'".',';
	}
...
if(!isset($searchedfields['allcat']) && $searchedfields['cat2']!=''){
	$fieldsStrWhere .= " and ".$tablesname.".Cat2 IN ('".$searchedfields['cat2']."') ";
}
