...
* @live URL: http://statestats.rand.org/stats/govtfin/cffus_det.html
********************************************/
...
if(!isset($_SESSION['cffus_det']) || (isset($_SESSION['cffus_det']) && !isset($_SESSION['cffus_det']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: cffus_det.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: cffus_det.php');
		exit;
	}
}

$searchedfields = $_SESSION['cffus_det'];
...
if(!empty($searchedfields['areacode'])){
	foreach($searchedfields['areacode'] as $areas){
		$areasAll.="'".$areas."'".',';
	}
...
if(!empty($searchedfields['agency'])){
	...
	$fieldsStrWhere .= " and ".$tablesname.".Agency IN (".$agencyStr.") ";
}
...
$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area in (".$areaStr.") and ".$tablesname.".Category IN (".$categoryStr.")  ".$joinStr." limit 4000";
