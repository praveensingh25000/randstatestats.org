...
if(!isset($_SESSION['cff']) || (isset($_SESSION['cff']) && !isset($_SESSION['cff']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: cff.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: cff.php');
		exit;
	}
}

$searchedfields = $_SESSION['cff'];
...
$agencyAll = $agencyStr = $areasAll = $areaStr = $cat1All = $cat1Str = $categoryAll = $categoryStr = $stateStr = $stateStrAll = '';
...
if(!empty($searchedfields['agency'])){
	foreach($searchedfields['agency'] as $agencys){
		$agencyAll.="'".$agencys."'".',';
	}
	$agencyStr = substr($agencyAll, 0, -1);
}
...
if($areaStr=='' && $categoryStr== '' && $agencyStr=='') {

	$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 and ".$tablesname.".Category = 'Total, all categories' and ".$tablesnamearea.".agency_code = 'All'  and ".$tablesnamecat.".program_code = 'All' ".$joinStr." and ".$tablesname.".Area < 100 and Units = 'Dollars' limit 1000";


} else {
	$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Area in (".$areaStr.") and ".$tablesname.".Category IN (".$categoryStr.") and ".$tablesname.".Agency IN (".$agencyStr.") ".$joinStr." limit 4000";
}
