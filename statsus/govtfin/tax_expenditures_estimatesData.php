...
if(!isset($_SESSION['tax_expenditures_estimates']) || (isset($_SESSION['tax_expenditures_estimates']) && !isset($_SESSION['tax_expenditures_estimates']['dbid']))){
	
	if(isset($_REQUEST['dbc']) && $_REQUEST['dbc']!='') {
		header('location: tax_expenditures_estimates.php?dbc='.$_REQUEST['dbc'].'');			
		exit;
	} else {
		header('location: tax_expenditures_estimates.php');
		exit;
	}
}

$searchedfields = $_SESSION['tax_expenditures_estimates'];
...
if(!empty($searchedfields['Function'])){
	$functionAll = $functionStr='';
	foreach($searchedfields['Function'] as $function){
		$functionAll.="'".$function."'".',';
	}
	$functionStr = substr($functionAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".Function IN (".$functionStr.") ";
}

if(!empty($searchedfields['Provision'])){
	$provisionAll = $provisionStr='';
	foreach($searchedfields['Provision'] as $provision){
		$provisionAll.="'".$provision."'".',';
	}
	$provisionStr = substr($provisionAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".Provision IN (".$provisionStr.") ";
}

if(!empty($searchedfields['Category'])){
	$categoryAll = $categoryStr='';
	foreach($searchedfields['Category'] as $category){
				...
				<table id="" class="data-table">
					...
					if(isset($columns['quatersascolumns']) && isset($columns['yearsasrows']) && isset($timeIntervalSettings) && ($timeIntervalSettings['time_format'] == 'SQ-SY-EQ-EY' || $timeIntervalSettings['time_format'] == 'SY-EY')){		// When data is as year q1, q2, q3, q4
						...
						foreach($searchedData as $keyData => $rowData){ 
							...
							if(array_key_exists('Area', $rowData) && array_key_exists('Category', $rowData) && array_key_exists('Year', $rowData)){
							...
							} else if((array_key_exists('Function', $rowData) || array_key_exists('function', $rowData)) && (array_key_exists('Provision', $rowData) || array_key_exists('provision', $rowData)) && (array_key_exists('Category', $rowData) || array_key_exists('category', $rowData)) && (array_key_exists('Year', $rowData) || array_key_exists('year', $rowData))){
								
								$origin = (array_key_exists('Function', $rowData))?$rowData['Function']:$rowData['function'];
								$Provision = (array_key_exists('Provision', $rowData))?$rowData['Provision']:$rowData['provision'];
								...
								$columnsArrayData[$origin][$Provision][$category][$year] = $rowData;
