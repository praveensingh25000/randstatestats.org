...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: us_pensionsData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: us_pensionsData.php');
		exit;
		}
	}
}
...
$statesResult = $admin->getDistinctColumnValuesUniversal($tablesnamearea , 'areastate', 'areast');
if(mysql_num_rows($statesResult)>0){
	while($rowState = mysql_fetch_assoc($statesResult))
	$filtervaluesJsonArray[] = array('id' => $rowState['areast'], 'name' => $rowState['areastate']);
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="us_pensions">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_choose_state_pensioner'])){ echo $lang['lbl_choose_state_pensioner']; } ?> </span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_choose_fund_type'])){ echo $lang['lbl_choose_fund_type']; } ?></span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
						<select size="4" name="sector" id="sector" class="required">
							<option value="all" selected="">All fund types</option>
							<option value="0">State funds</option>
							<option value="1">County funds</option>
							<option value="2">City/Municipal/Township funds</option>
							<option value="3">Special District/School funds</option>
						</select>
					</div>
				  </div>
				<!-- /form -->
				...
				$(document).ready(function() {	
					...
					$('#sector').change(function(){
						...
						if(states!='' && states != "NULL"){
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/govtfin/us_pensions_ajax.php',
								type: 'post',
							...
							//alert("<?php if(isset($lang['lbl_choose_state_first_us_pension'])){ echo $lang['lbl_choose_state_first_us_pension']; } ?>");
						}
					});
