...
if(isset($_POST['getresults'])){
	...
	$_SESSION['tax_expenditures_estimates'] = $_POST;
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: tax_expenditures_estimatesData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: tax_expenditures_estimatesData.php');
		exit;
		}
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="tax_expenditures_estimates">
					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_Please_choose_function'])){ echo $lang['lbl_Please_choose_function']; } ?> </span>
						</p>
						<div class="table-div">							
							<div class="left">							
								<table cellspacing="1" cellpadding="7" border="0" class="collapse" id="grid_view" width="100%">
								
								<?php foreach($functionName as $key => $value){ ?>
									<tr>
										<td>
											<input type="radio" value="<?php echo $value['funcode'];?>" id="function<?php echo $value['funcode'];?>" name="Function[]">&nbsp;&nbsp;<?php echo $value['funname'];?>
											<script type="text/javascript">
											$(document).ready(function() {					
												$("#function<?php echo $value['funcode'];?>").click(function(){
													var functioncode = jQuery("#function<?php echo $value['funcode'];?>").val();	
													if(functioncode !='') {								
														loader_show();
														jQuery('#timePeriod').hide();
														jQuery('#submitButtons').hide();
														jQuery.ajax({
															url: URL_SITE+'/statsus/govtfin/tax_expenditures_estimatesAjax.php',
															type: 'post',
															data: 'dbid=<?php echo $dbid; ?>&functioncode='+functioncode,
