<?php
/******************************************
* @created on Feb 28, 2013, Mar 12, 2013
* @Package: Rand
* @Developer: sandeep kumar, Pragati garg
* @live Site URL For This Page: http://statestats.rand.org/stats/govtfin/govtfin.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		if(isset($dbnameurl) && $dbnameurl!='') {
			header('location: govtfinData.php?dbc='.$_REQUEST['dbc'].'');			
			exit;
		} else {
		header('location: govtfinData.php');
		exit;
		}
	}
}
...
$statesResult = $admin->searchLikeUniversalEqual($tablesnamearea , 'areaco', '0', 'order by areaname');
if(mysql_num_rows($statesResult)>0){
	while($rowState = mysql_fetch_assoc($statesResult))
	$filtervaluesJsonArray[] = array('id' => $rowState['areast'], 'name' => $rowState['areaname']);
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="govtfin">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_choose_state_govt'])){ echo $lang['lbl_choose_state_govt']; } ?> </span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_choose_govt_type'])){ echo $lang['lbl_choose_govt_type']; } ?></span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
						<select size="4" name="sector" id="sector" class="required">
							<option value="1">Counties</option>
							<option value="2">Cities/Townships</option>
							<option value="4">Special Districts</option>
							<option value="5">School Districts</option>
						</select>
					</div>
				  </div>
				<!-- /form -->
				...
				$(document).ready(function() {	
					...
					$('#sector').change(function(){
						...
						if(states!='' && states != "NULL"){
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statsus/govtfin/govtfinAjax.php',
								type: 'post',
							...
							alert("<?php if(isset($lang['lbl_choose_state_govt'])){ echo $lang['lbl_choose_state_govt']; } ?>");
						}
					});
