...
if(isset($_GET['plan_name']) && isset($_GET['user_type']) && isset($_GET['db_name'])){
	...
	if(isset($_REQUEST['is_trail_account']) && $_REQUEST['is_trail_account'] == 'no'){ // Purchased User

		if(isset($_REQUEST['usonly']) && $_REQUEST['usonly'] == 'yes'){
			$onlyus = 1;
		...
		echo "Price in Percentage: ".$pricepercentage." ( 0 - No, 1 - Yes )<br/><br/>";*/ 
		...
		if($onlyus == 0){
			...
			if($totalstates == 1){
				$surcharge = $surchargeonestateus;
			} else if($totalstates == 2){
				$surcharge = $surchargetwostateus;
			} else if($totalstates == 3){
				$surcharge = $surchargethreestateus;
			}else {
				$surcharge = 0;
			}
		}
		...
		$validity = (isset($_REQUEST['validity']) && $_REQUEST['validity']>0)?$_REQUEST['validity']:'365';

		$daysleft = (isset($_REQUEST['daysleft']) && $_REQUEST['daysleft']>0)?$_REQUEST['daysleft']:'365';

		$firstpayment = (isset($_REQUEST['firstpayment']) && $_REQUEST['firstpayment']>0)?$_REQUEST['firstpayment']:0;
		$secondpayment = (isset($_REQUEST['secondpayment']) && $_REQUEST['secondpayment']>0)?$_REQUEST['secondpayment']:0;
		$thirdpayment = (isset($_REQUEST['thirdpayment']) && $_REQUEST['thirdpayment']>0)?$_REQUEST['thirdpayment']:0;

		if($firstpayment != 0){
			$basepricetotal = $firstpayment;
		}
		...
		if($pricepercentage == 0 && $user_type != '6'){	// Percentage amount
		...
		} else if($pricepercentage == 0 && $user_type == '6'){	// Mutiple Users percentage amount
			...
			if($firstpayment != 0){
				$basepricetotal = $firstpayment;
				$surcharegtotal = ($basepricetotal * (($surchargeindividuallogin)/100));
			}
		...
		if($daysleft != $validity){
			$surcharegtotal = ($surcharegtotal/$validity) * $daysleft;
		}
		...
		if($user_type != '6'){
			...
			echo "Days left: ".$daysleft."<br/><br/>";

			echo "Validity: ".$validity."<br/><br/>";*/
		}
		...
		if($firstpayment == 0){
			...
			$chargeableamount = $chargeableamount - $firstpayment;
		}
		...
		$jsonArray['number_of_users']		= $number_of_users;
