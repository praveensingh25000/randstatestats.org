...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: teachsalbenData.php');
		exit;
	}
}
...
foreach($stateToName as $key => $value){
	if($value['areacode'] != 00){
		...
		$filtervaluesStr .= '<p>'.str_replace("'","",$value['areaname'])."</p>";	
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				<!-- form -->
				<input type="hidden" name="session_setter" value="teachsalben"/>

					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_choose_county_teacher_bin'])){ echo $lang['lbl_choose_county_teacher_bin']; } ?> </span>
						</p>
						...
						<div class="wdthpercent20 left pT15 pL20">
					...
					$(document).ready(function() {							
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								...
								jQuery.ajax({
									url: URL_SITE+'/statsca/education/teachsalbenAjax.php',
									type: 'post',
