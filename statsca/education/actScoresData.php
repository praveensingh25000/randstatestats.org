...
$tablesname = "ca_act_scores";
$tablenamecounties = "ca_counties";

if(!isset($_SESSION['actscores']) || (isset($_SESSION['actscores']) && !isset($_SESSION['actscores']['dbid']))){
	header('location: actScores.php');
}

$searchedfields = $_SESSION['actscores'];
...
if(isset($searchedfields['category'])){
	$categoryStr = '';
	foreach($searchedfields['category'] as $keyCategory => $category){
		$categoryStr .= "'".$category."',";
	}

	$categoryStr = substr($categoryStr, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".category IN (".$categoryStr.")";
}

if(isset($searchedfields['schooldistrict'])){
	$schooldistrictStrAll = $schooldistrictStr = '';
	foreach($searchedfields['schooldistrict'] as $keyCategory => $schooldistrict){
		$schooldistrictStrAll .= "'".$schooldistrict."',";
	}

	$schooldistrictStr = substr($schooldistrictStrAll, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".school_number IN (".$schooldistrictStr.")";
}

if(isset($searchedfields['us_states'])){
	$us_states = explode(';', $searchedfields['us_states']);
	$usstates = implode(",", $us_states);

	$fieldsStrWhere .= " and ".$tablesname.".county_number IN (".$usstates.")";
}
