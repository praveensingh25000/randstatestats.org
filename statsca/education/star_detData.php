<?php
/******************************************
* @Modified on May 7, 2013
* @Package: Rand
* @Developer: Pragati Garg
* @URL : http://www.ideafoundation.in
* @live URL: http://ca.rand.org/stats/education/star_det.html
********************************************/
...
$tablesname				= "star1det";
$tablesnamecats			= "stardet_cats";
$tablesnamecd			= "cd_new";
$tablesnamecds			= "cds";
$tablesnamecounties		= "ca_counties";

if(!isset($_SESSION['star_det']) || (isset($_SESSION['star_det']) && !isset($_SESSION['star_det']['dbid']))){
	header('location: star_det.php');
}

$searchedfields = $_SESSION['star_det'];
...
if(isset($searchedfields['Category'])){
	$category = implode(',', $searchedfields['Category']);
	$fieldsStrWhere .= " and ".$tablesname.".Category IN (".$category.")";
}

if(isset($searchedfields['Cat1'])){
	$cat1Str = '';
	foreach($searchedfields['Cat1'] as $keyCat => $Cat1){
		$cat1Str .= "'".$Cat1."',";
	}

	$cat1Str = substr($cat1Str, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".Cat1 in (".$cat1Str.")";
}

if(isset($searchedfields['Cat2'])){
	$cat2Str = '';
	foreach($searchedfields['Cat2'] as $keyCat => $Cat2){
		$cat2Str .= "'".$Cat2."',";
	}

	$cat2Str = substr($cat2Str, 0, -1);
	$fieldsStrWhere .= " and ".$tablesname.".Cat2 in (".$cat2Str.")";
}

$cds = implode(',', $searchedfields['schooldistrict']);

$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".CDS in (".$cds.") ".$joinStr." limit 4000";
