...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: api1999Data.php');
		exit;
	}
}
...
$allCategoryDetail_res  = $admin->getTableDataUniversal($tablesnamecounties);
$stateToName = $dbDatabase->getAll($allCategoryDetail_res);

foreach($stateToName as $key => $value)
if($value['areacode'] != 00)
$filtervaluesJsonArray[] = array('id' => (int)$value['areacode'], 'name' => $value['areaname']);

$filtervaluesJson = json_encode($filtervaluesJsonArray);
//echo "<pre>";print_r($filtervaluesJsonArray);echo "</pre>"; die;

$filtervaluesJsonArrayCats = array();
$catsStr = '';
foreach($CatTitles as $keyC => $valueC)	{
   $filtervaluesJsonArrayCats[] = array('id' => $keyC, 'name' => $valueC);
   $catsStr .= $valueC.'<br/>';
}

$filtervaluesJsonCats = json_encode($filtervaluesJsonArrayCats);
?>
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				<!-- form -->
				<input type="hidden" name="session_setter" value="api"/>

				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_search_cat_lep'])){ echo $lang['lbl_search_cat_lep']; } ?> </span>
					</p>

					<div class="wdthpercent35 left">
						<div class="table-div">
							<input type="text" id="search_criteria_duplicatescats" name="cats"/>
						</div>
					</div>

					<div class="wdthpercent20 left pT15 pL10">
						<a id="viewallcats" href="javascript:;" onclick="javascript: popup_window('<?php echo $catsStr;?>');">See a list</a>
				...
				<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_search_district_lep'])){ echo $lang['lbl_search_district_lep']; } ?> </span>
						</p>
						...
						<div class="wdthpercent20 left pT15 pL10">
							<a id="viewall" href="javascript:;">See a list</a>
							<script type="text/javascript">
							jQuery(document).ready(function(){
								jQuery('#viewall').click(function(){
									window.open("<?php echo URL_SITE; ?>/allValues.php?table=<?php echo $tablesnamecounties;?>&column=areaname", 'popUpWindow','height=400, width=400, left=300, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');
								});
							});
							</script>
						</div>
					...
					$(document).ready(function() {							
						$("#search_criteria_duplicatescats").tokenInput(<?php echo $filtervaluesJsonCats; ?>, {		
							preventDuplicates: true,							
							onAdd: function (item) {
								var states = jQuery('#search_criteria_duplicates').val();
								var cats = jQuery('#search_criteria_duplicatescats').val();
								if(states != ''){
									loader_show();
									jQuery('#timePeriod').hide();
									jQuery('#submitButtons').hide();
									jQuery.ajax({
										url: URL_SITE+'/statsca/education/api1999Ajax.php',
										type: 'post',
