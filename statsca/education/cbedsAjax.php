...
$allCategoryDetailages_res  = $admin->getDistinctColumnValuesUniversal($tablesnamecats , $column='catname', $columns = "catcode", $limit = "");

if(mysql_num_rows($allCategoryDetailages_res) >0 ){

	$data .= '<br><div class="form-div">
			  <p><span class="choose">'.((isset($lang['lbl_cat']))?$lang['lbl_cat']:'').'</span>&nbsp;&nbsp;&nbsp;<input type="checkbox" id="checkallCategory">&nbsp;All
			  ...
			  <script type="text/javascript">
				jQuery(document).ready(function(){
					jQuery("#checkallCategory").click(function(){
						...
						if(checkedd){
							jQuery(".categoryAll").attr("checked", "true");	
						}else{
							jQuery(".categoryAll").removeAttr("checked");
						}
					});
	...
	while($categoryDetail = mysql_fetch_assoc($allCategoryDetailages_res)){
		$data .= '<input type="checkbox" class="categoryAll required" name="Category[]" value = "'.$categoryDetail['catcode'].'">&nbsp;'.$categoryDetail['catname'].'<br/>';
	}
	$data .= '</select></div>';
