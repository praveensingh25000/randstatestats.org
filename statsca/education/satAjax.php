...
if(mysql_num_rows($allCategoryDetailages_res) >0 ){
	...
	$data .= '<br><div class="form-div">
					<p><span class="choose">'.((isset($lang['lbl_categories']))?$lang['lbl_categories']:'').'</span>&nbsp;&nbsp;</p></div>';
	
	$data .= '<div>';
	
	while($categoryDetail = mysql_fetch_assoc($allCategoryDetailages_res)){
	$data .= '<input type="checkbox" class="required" name="Category[]" value = "'.$categoryDetail['catcode'].'">&nbsp;'.$categoryDetail['catname'].'<br/>';
	}
