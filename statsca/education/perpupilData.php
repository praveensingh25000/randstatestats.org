...
* @live URL: http://ca.rand.org/stats/education/perpupil.html
********************************************/
...
if(!isset($_SESSION['perpupil']) || (isset($_SESSION['perpupil']) && !isset($_SESSION['perpupil']['dbid']))){
	header('location: perpupil.php');
}

$searchedfields = $_SESSION['perpupil'];

//echo "<pre>";print_r($searchedfields);die;
...
$cdsStr = $cdsAll = $categoryAll = $categoryStr = $stateStr = $stateStrAll = $areasAll = $cat1All = $cat1Str = $areaStr='';

$columnstobefetched = substr($columnstobefetched, 0, -1);



if(isset($searchedfields['cds'])){
	
	$cds = implode(',', $searchedfields['cds']);

	if(isset($searchedfields['allstates']) && isset($searchedfields['districtavg']) && isset($searchedfields['statedistrictavg'])){

		$fieldsStrWhere .= " and ( (".$tablesname.".CDS != '0' and ".$tablesname.".county = '".$searchedfields['us_states']."') or (".$tablesname.".CDS = '".$searchedfields['us_states']."' and ".$tablesname.".county = '".$searchedfields['us_states']."') or (".$tablesname.".CDS = '0' and ".$tablesname.".county = '0') )";

	} else if(!isset($searchedfields['allstates']) && isset($searchedfields['districtavg']) && isset($searchedfields['statedistrictavg'])){

		$fieldsStrWhere .= " and ( (".$tablesname.".CDS in (".$cds.")) or (".$tablesname.".CDS = '".$searchedfields['us_states']."' and ".$tablesname.".county = '".$searchedfields['us_states']."') or (".$tablesname.".CDS = '0' and ".$tablesname.".county = '0') )";

	} else if(!isset($searchedfields['allstates']) && !isset($searchedfields['districtavg']) && isset($searchedfields['statedistrictavg'])){

		$fieldsStrWhere .= " and (".$tablesname.".CDS in (".$cds.") or (".$tablesname.".CDS = '0' and county = '0') )";

	} else if(!isset($searchedfields['allstates']) && isset($searchedfields['districtavg']) && !isset($searchedfields['statedistrictavg'])){

		$fieldsStrWhere .= " and (".$tablesname.".CDS in (".$cds.") or (".$tablesname.".CDS = '".$searchedfields['us_states']."' and county = '".$searchedfields['us_states']."') )";
	...
	if(isset($searchedfields['allstates']) && isset($searchedfields['districtavg']) && isset($searchedfields['statedistrictavg'])){
		...
		$fieldsStrWhere .= " and ( (".$tablesname.".CDS = '".$searchedfields['us_states']."' and ".$tablesname.".county = '".$searchedfields['us_states']."') or (".$tablesname.".CDS = '0' and ".$tablesname.".county = '0') )";

	} else if(!isset($searchedfields['allstates']) && !isset($searchedfields['districtavg']) && isset($searchedfields['statedistrictavg'])){

		$fieldsStrWhere .= " and ( (".$tablesname.".CDS = '0' and county = '0') )";

	} else if(!isset($searchedfields['allstates']) && isset($searchedfields['districtavg']) && !isset($searchedfields['statedistrictavg'])){

		$fieldsStrWhere .= " and ( (".$tablesname.".CDS = '".$searchedfields['us_states']."' and county = '".$searchedfields['us_states']."') )";

	}

}

if(isset($searchedfields['us_states']) && strlen($searchedfields['us_states']) == '1') {
	$usstate=str_pad($searchedfields['us_states'], 2, '0', STR_PAD_LEFT);
} else {
	$usstate=$searchedfields['us_states'];
}

$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesnamecounties.".areacode = '".$usstate."' ".$joinStr." limit 4000";
