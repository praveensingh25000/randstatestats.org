...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: actScoresData.php');
		exit;
	}
}
...
$allCategoryDetail_res  = $admin->getTableDataUniversal($tablenamecounties);
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_search_counties'])){ echo $lang['lbl_search_counties']; } ?> </span>
						</p>
						...
						<div class="wdthpercent20 left pT15 pL10">
							...
							jQuery(document).ready(function(){
								jQuery('#viewall').click(function(){
									window.open("<?php echo URL_SITE; ?>/allValues.php?table=<?php echo $tablenamecounties;?>&column=areaname", 'popUpWindow','height=400, width=400, left=300, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');
								});
							});
							</script>
						</div>
					...
					$(document).ready(function() {							
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								...
								jQuery.ajax({
									url: URL_SITE+'/statsca/education/actScoresAjax.php',
									type: 'post',
