...
if(!isset($_SESSION['deaths']) || (isset($_SESSION['deaths']) && !isset($_SESSION['deaths']['dbid']))){
	header('location: deaths.php');
	exit;
}

$searchedfields = $_SESSION['deaths'];


if($searchedfields['categories'] == ""){
	$_SESSION['errormsg'] = "Please enter a category.";
	header('location: deaths.php');
	exit;
}

if($searchedfields['zips'] == "" && $searchedfields['areas'] == ""){
	$_SESSION['errormsg'] = "Please enter a county or enter zips.";
	header('location: deaths.php');
	exit;
}
...
if(isset($searchedfields['areas'])){
	$strArea = "";
	$areas = explode(';', $searchedfields['areas']);
	foreach($areas as $key => $area){
		$strArea .= (int)$area.",";
	}
	$strArea = substr($strArea, 0, -1);
	$fieldsStrWhere .= " and deaths.Area in (".$strArea.")";
}

if(isset($searchedfields['categories'])){
	$strArea = "";
	$categories = explode(';', $searchedfields['categories']);
	$categories = implode(",", $categories);
	
	$fieldsStrWhere .= " and deaths.Category in (".$categories.")";
}


$sql = "";

if(isset($searchedfields['areas']) && $searchedfields['areas']!=""){
	$sql = "select ca_counties.areaname as Area, ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere."  ".$joinStr." limit 4000";
}
...
if(isset($searchedfields['zips']) && $searchedfields['zips']!=""){

	$zips = explode(';', $searchedfields['zips']);
	$zips = implode(",", $zips);
	...
	$tablesStrZip1 = str_replace('deaths,','deathszip1,', $tablesStr);
	$tablesStrZip1 = str_replace('ca_counties,','', $tablesStrZip1);

	$columnstobefetchedZip1 = str_replace('deaths.','deathszip1.', $columnstobefetched);

	$sqlZip1 = "select deathszip1.Area, ".$columnstobefetchedZip1." from ".$tablesStrZip1." where 1 and deathszip1.Area in (".$zips.") and  deathszip1.Category in (".$categories.") and deathszip1.Category = deaths_cats.catcode and deathszip1.Units = units_codes.unitscode limit 4000";

	$tablesStrZip2 = str_replace('deaths,','deathszip2,', $tablesStr);
	$tablesStrZip2 = str_replace('ca_counties,','', $tablesStrZip2);

	$columnstobefetchedZip2 = str_replace('deaths.','deathszip2.', $columnstobefetched);

	$sqlZip2 = "select deathszip2.Area, ".$columnstobefetchedZip2." from ".$tablesStrZip2." where 1 and deathszip2.Area in (".$zips.") and  deathszip2.Category in (".$categories.") and deathszip2.Category = deaths_cats.catcode and deathszip2.Units = units_codes.unitscode limit 4000";
