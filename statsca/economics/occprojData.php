...
if(!isset($_SESSION['occproj']) || (isset($_SESSION['occproj']) && !isset($_SESSION['occproj']['dbid']))){
	header('location: occproj.php');
}

$searchedfields = $_SESSION['occproj'];
...
if(!empty($searchedfields['Category1'])){
	$categoryAll = $categoryStr ='';
	foreach($searchedfields['Category1'] as $categories){
		$categoryAll.="'".$categories."'".',';
	}
	$categoryStr = substr($categoryAll, 0, -1);
}

$sql = "select ".$columnstobefetched." from ".$tablesStr." where 1 ".$fieldsStrWhere." and ".$tablesname.".Category IN (".$categoryStr.") and ".$tablesname.".Area IN (".$areaStr.") ".$joinStr." limit 4000";
...
<section id="container">
	 ...
	 <div class="main-cell">
		...
		if(count($searchedData)>0) {
			...
			<div id="" class="right">
				<?php
				if(((isset($_GET['graph']) && $_GET['graph']=='gridview')) || !isset($_GET['graph'])) { ?>
					<ul class="submenu">
						<!-- DOWNLOAD ------>
