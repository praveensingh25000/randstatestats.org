...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: occprojData.php');
		exit;
	}
}
...
$area_res						= $admin->getDistinctColumnValuesUniversal($tablesname , $column='Area', $columns = "", $limit = "");
$stateToName					= $dbDatabase->getAll($area_res);
...
$categoryDetail					= $dbDatabase->getAll($allCategoryDetail_res);

foreach($categoryDetail as $key => $category)
$categoryvaluesJsonArray[]		= array('id' => $category['catcode'], 'name' => $category['catename']);
$categoryvaluesJson				= json_encode($categoryvaluesJsonArray);
?>
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="occproj">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_areas'])){ echo $lang['lbl_select_areas']; } ?></span>
					</p>
					<div class="table-div">
						<?php if(!empty($filtervaluesJsonArray)) { ?>
							<?php foreach($filtervaluesJsonArray as $area) { ?>
							<p class="pT5 PB5">
								<input value="<?php echo $area['name']?>" class="required" type="checkbox" id="Area" name="Area[]"/>
								<label for="<?php echo $area['name']?>"><?php echo $area['name']?></label>
							</p>
							<?php } ?>
						<?php } ?>
					</div>
				...
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_any_one_category'])){ echo $lang['lbl_select_any_one_category']; } ?></span>&nbsp;&nbsp;
					</p>
					<div class="table-div">
						<div class="table-div">
							<input type="text" id="search_criteria_duplicates_category" name="Category"/>
						</div>
					</div>
				  </div>
				<!-- /form -->
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates_category").tokenInput(<?php echo $categoryvaluesJson; ?>, {
						tokenLimit :1,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: URL_SITE+'/statsca/economics/occprojAjax.php',
								type: 'post',
