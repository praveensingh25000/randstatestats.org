<?php
/******************************************
* @Modified on March 05, 2013, Mar 08,2013
* @Package: Rand
* @Developer: Praveen Singh, Pragati garg
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: trafficData.php');
		exit;
	}
}
...
$allCategoryDetail_res  = $admin->getTableDataUniversal($tablesnamecds);
...
foreach($stateToName as $key => $value) {
	...
	$filtervaluesArray[] = $value['areacode'];
}
$fielsvalueStr=implode(';',$filtervaluesArray);
$filtervaluesJson = json_encode($filtervaluesJsonArray);
//echo "<pre>";print_r($fielsvalueStr);echo "</pre>"; die;
?>
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<div id="" class="">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="traffic"/>

					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_choose_county'])){ echo $lang['lbl_choose_county']; } ?> &nbsp;&nbsp;&nbsp;
						   <!-- <input id="search_criteria_duplicates_all" type="checkbox" value="<?php echo $fielsvalueStr;?>" name="allow_all" /> All</span> -->
						</p>
						<div class="table-div">
							<input class="search_field_required_div" type="text" id="search_criteria_duplicates" name="us_states"/>		
						</div>
					</div>
					<!-- /form -->
					...
					$(document).ready(function() {							
						jQuery("#search_criteria_duplicates_all").click(function(){
							...
							if(jQuery(this).is(":checked")){
								...
								jQuery("#search_criteria_duplicates").tokenInput("toggleDisabled");
								jQuery(".search_field_required_div").removeClass("required");

								var states = jQuery('#search_criteria_duplicates_all').val();
								loader_show();
								jQuery('#timePeriod').hide();
								jQuery('#submitButtons').hide();
								
								jQuery.ajax({
									url: URL_SITE+'/statsca/community/trafficAjax.php',
									type: 'post',
								...
								jQuery("#search_criteria_duplicates").tokenInput("toggleDisabled");		
								jQuery(".search_field_required_div").addClass("required");
