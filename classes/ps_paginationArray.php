...
class PS_PaginationArray {
	...
	function paginate() {
		...
		$rs = @array_slice($this->arraydata, $this->offset, $this->rows_per_page,true);
		return $rs;
	}
	...
	function renderNext($tag='Next') {
		...
		$imgR='/images/arrowR.gif';
		if($this->page < $this->max_pages) {
			return '<span><a href="'.$this->php_self.'?page='.($this->page+1).'&'.$this->append.'" class="txtred pR5">'.$tag.'</a>'.'<img src="'.$imgR.'"/></span>';
		}
	...
	function renderPrev($tag='Previous') {
		...
		$imgL='/images/arrowL.gif';
		if($this->page > 1) {
			return '<span><img src="'.$imgL.'"/> <a href="'.$this->php_self.'?page='.($this->page-1).'&'.$this->append.'" class="txtred pR5">'.$tag.'</a></span>';
		}
