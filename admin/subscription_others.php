...
<section id="container">
	 ...
	 <div class="main-cell">
							...
							<tbody>
								...
								<?php foreach($plans as $key => $planDetail){?>
									<tr>
										...
										<td align="left">
											<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=viewall&id=<?php echo base64_encode($planDetail['id']);?>"><?php echo $planDetail['user_type']; ?></a>
										</td>
										<td align="center">
											<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=viewall&id=<?php echo base64_encode($planDetail['id']);?>">View Plans</a> | 
											<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']); ?>&action=add&id=<?php echo base64_encode($planDetail['id']);?>">Add Plans</a>
										</td>
									</tr>
								<?php } ?>	
		...
		<?php if(isset($section_type) && $action_type=='viewall') {

			$planid						=		trim(base64_decode($_GET['id']));			
			$selectalltypeofplans_res   =		$admin->selectallsubscriptionsPlans($section_type,$planid	);
			...
			<div class="containerLadmin">
				<div style="clear: both; padding: 15px 0;">
					<h3><a href="subscriptions.php?type=plan">All Subscription Plans</a> >> 
						<span class="font14">
						<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=viewall&id=<?php echo base64_encode($planDetail['id']);?>"><?php echo ucwords($section_type);?></a> >>  All Plan List
						<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']); ?>&action=add&id=<?php echo base64_encode($planDetail['id']);?>" style='float:right;'>Add Plans</a></span>
					</h3>
					<fieldset class="pT15">
						<table cellspacing="0" cellpadding="4" border="1" class="collapse" id="grid_view" width="100%">
							<tbody>
								<tr>
									...
									<?php } ?>
									<?php if($section_type!='single' && $section_type!='multiple'){?>
								...
								<?php if(!empty($alltypeofplansDetail)){							
									...
									<tr>									
										...
										<td align="left">
											<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=view&id=<?php echo base64_encode($planDetail['id']);?>&planid=<?php echo base64_encode($allplanDetail['id']);?>"><?php echo $allplanDetail['plan_name']; ?></a>
										</td>
										...
										<td align="center">
											<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=view&id=<?php echo base64_encode($planDetail['id']);?>&planid=<?php echo base64_encode($allplanDetail['id']);?>">View</a> | 
											<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=edit&id=<?php echo base64_encode($planDetail['id']);?>&planid=<?php echo base64_encode($allplanDetail['id']);?>">Edit</a> | 
		...
		<?php if(isset($section_type) && isset($action_type) && ($section_type!='single' || $section_type!='multiple') && ($action_type=='add' || $action_type=='edit' || $action_type=='view')) {?>
			<!-- ADD All SUBSCRIPTIONS PLAN  ---------------------------------------->
			<div class="containerLadmin">
				<div style="clear: both; padding: 15px 0;">
					<h3>
						<a href="subscriptions.php?type=plan">All Subscription Plans</a><span class="font14"> >> <a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=viewall&id=<?php echo base64_encode($planDetail['id']);?>"><?php echo ucwords($section_type);?></a> >> 
							<?php if($action_type=='add'){?>Add Plan Detail	
							<?php } else if($action_type=='view'){?>
								<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=view&id=<?php echo base64_encode($planDetail['id']);?>&planid=<?php echo $_GET['planid'];?>"><?php if(!empty($particularSubscriptionDetail)) { echo ucwords($particularSubscriptionDetail['plan_name']);}?> </a> >> View Plan Detail 
								<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=edit&id=<?php echo base64_encode($planDetail['id']);?>&planid=<?php echo $_GET['planid'];?>" style='float:right;'>Edit Plan Detail</a>
							<?php } else if($action_type=='edit'){?> 
								<a href="subscription_others.php?type=<?php echo strtolower($planDetail['user_type']);?>&action=view&id=<?php echo base64_encode($planDetail['id']);?>&planid=<?php echo $_GET['planid'];?>"><?php if(!empty($particularSubscriptionDetail)) { echo ucwords($particularSubscriptionDetail['plan_name']);}?> </a> >> Edit Plan Detail  
							<?php } ?>
						</span>
					</h3>
					...
					<fieldset class="pT15">			
						<form <?php if($action_type=='add'){?> name="frmaddall_subscription_plan" id="frmaddall_subscription_plan" <?php } else { ?> id="frmeditall_subscription_plan" name="frmeditall_subscription_plan" <?php } ?>  method="post" action="subscriptionsAction.php">
							...
							<div style="padding: 10px 0;">
								...
								<input type="text" onchange="checknumberofusersPlanAvailability('<?php echo $section_type;?>','number_of_users_<?php echo $section_type;?>');" id="number_of_users_<?php echo $section_type;?>" placeholder="enter number of users" name="number_of_users" class="required" value="" />
								<?php } else if($action_type=='edit'){?>
								<input type="text" onchange="checknumberofusersPlanAvailability('<?php echo $section_type;?>','number_of_users_<?php echo $section_type;?>');" id="number_of_users_<?php echo $section_type;?>" placeholder="enter number of users" name="number_of_users" class="required" value="<?php if(!empty($particularSubscriptionDetail)) { echo $particularSubscriptionDetail['number_of_users'];}?>" />
							...
							<legend style="background: #cccccc; font-size: 14px; padding: 5px;"> Institution Type </legend>
							<div class="pT10 pB10">
							...
							<div id="show_selected_database_values" style="">
								...
								<div style="padding: 10px 0;">
									...
									<input type="text" onchange="checkPlanValidity('<?php echo $section_type;?>','validity_<?php echo $section_type;?>');" id="validity_<?php echo $section_type;?>" placeholder="enter validity in days" name="validity" class="required" value="<?php if(!empty($particularSubscriptionDetail)) { echo $particularSubscriptionDetail['validity'];}?>" />&nbsp;Days
									<?php } else if($action_type=='view'){ if(!empty($particularSubscriptionDetail)) echo $particularSubscriptionDetail['validity'];?>&nbsp;Days
									<?php } ?>
								</div>
								...
								<div style="padding: 10px 0;">
									<?php if($action_type=='add'){ ?>
									<input type="text" placeholder="enter discount amount" name="discounts" class="" value="" /><span style="color:blue;font-style:oblique;">press tab for discount.</span>	
									<?php } else if($action_type=='edit'){?>
									<input type="text" id="claculatediscountfunction_<?php echo $particularSubscriptionDetail['plan_type_id']?>" onchange="claculateDiscountFunction(<?php echo $particularSubscriptionDetail['plan_type_id']?>,'<?php echo $section_type;?>');" placeholder="enter discount amount" name="discounts" class="" value="<?php if(!empty($particularSubscriptionDetail)) { echo $particularSubscriptionDetail['discounts'];}?>" /><span style="color:blue;font-style:oblique;">press tab for discount.</span>	
									...
									if(!empty($databases)){ 
										...
										foreach($databases_slice as $key => $databaseDetail){?>
											<p class="pT5 pB5 pL20">
												<span style="background: #EAF6B0;"><?php echo ucwords($databaseDetail['database_label']);?></span>&nbsp;
												<?php if($action_type=='edit'){?>
												<input placeholder="enter amount in USD" type="text" name="<?php echo $databaseDetail['id']; ?>" class="wdth10 required" value="<?php if(!empty($new_array)) {
												foreach($new_array as $key => $dbvalues) {
												if($key == $databaseDetail['id'])
												echo $db_values= $dbvalues;
												}}?>" />
