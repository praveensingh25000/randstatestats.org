...
if(isset($_POST['update']))
{
	...
	$is_filter = (isset($_POST['filter_values']))?'Y':'N';

	if(isset($_POST['filtered_values'])){
		$filtered_values = explode(';', $_POST['filtered_values']);
		$filtered_values = serialize($filtered_values);
	} else {
		$filtered_values = '';
	}
		...
		foreach($tables_selected as $table_name_key=>$table_name){
			if(isset($_POST[$table_name])) {
				...
				foreach($table_coloums as $key=>$tc){
					$sql[] = '("' . mysql_real_escape_string($table_name) . '", "' . mysql_real_escape_string($tc) . '", "' . mysql_real_escape_string($search_id) . '", "'.$is_filter.'","' . mysql_real_escape_string($filtered_values) . '")';
				}
			}
		}
		if(isset($sql))
...
<section id="container">
	 <div class="main-cell">
		...
		switch($action){
			case 'add':
				include($DOC_ROOT.'admin/searchCriteriaActionNew.php');
				break;
			case 'edit':
