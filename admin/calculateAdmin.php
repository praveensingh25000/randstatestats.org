...
$jsonArray = array('totalamount' => 0, 'surcharge' => 0, 'minimumamount' => 0, 'minimumapplicable' => 0, 'states' => 0, 'surchargeapplicable' => 0, 'txtper' => '', 'txtdollar' => '', 'discounttxt' => '', 'discount' => 0, 'discountamount' => 0, 'actualamount' => 0);
...
if(isset($_GET['plan_name']) && isset($_GET['user_type']) && isset($_GET['db_name'])){
	...
	$actualamount = $chargeableamount;

	if(isset($_REQUEST['discountoffered']) && $_REQUEST['discountoffered']>0){

		$discount = (float)$_REQUEST['discountoffered'];
		...
		$jsonArray['discountamount']	= $discountamount;
		$jsonArray['discount']			= $discount;
		$jsonArray['discounttxt']		= $discounttxt;

	} else if(isset($discounts[$planyears]) && $discounts[$planyears]>0){
	...
	$jsonArray['totalamount']			= $chargeableamount;
	$jsonArray['surcharge']				= $surcharegtotal;
	...
	$jsonArray['actualamount']			= $actualamount;
