...
'--------------------------------------------------------------------------------------------------------------------------------------------
*/
function CallShortcutExpressCheckout($paymentAmounts, $currencyCodeType, $paymentType, $returnURL, $cancelURL,$planName)
{
	 global $URL_SITE;
	 ...
	 $nvpstr .="&PAYMENTREQUEST_0_ITEMAMT=".$paymentAmounts;	 
	 $nvpstr.='&PAYMENTREQUEST_0_AMT='.$paymentAmounts;
	
	 $count = 0;
	 $nvpstr.="&L_PAYMENTREQUEST_0_NAME".$count."=".urlencode($planName) ;	 
	 $nvpstr.="&L_PAYMENTREQUEST_0_DESC".$count."=".urlencode('User Account Activation');
	 $nvpstr.="&L_PAYMENTREQUEST_0_AMT".$count."=".($paymentAmounts);
...
$cancelURL, $shipToName, $shipToStreet, $shipToCity, $shipToState, $shipToZip, $phoneNum
)
{
	//------------------------------------------------------------------------------------------------------------------------------------
...
'--------------------------------------------------------------------------------------------------------------------------------------------
*/
function ConfirmPayment($FinalPaymentAmt,$token,$PayerID,$paymentmethod) {
	...
	*/
	...
	$paymentType = urlencode('D');
	$serverName = urlencode($_SERVER['SERVER_NAME']);
	$currencyCodeType = urlencode($_SESSION['currencyCodeType']);
	$PayerID = urlencode($PayerID);
	$nvpstr = '&TOKEN=' . $token . '&PAYERID='.$PayerID.'&PAYMENTACTION=SALE' . '&AMT=' . $FinalPaymentAmt;
	$nvpstr .= '&CURRENCYCODE=' . $currencyCodeType . '&IPADDRESS=' . $serverName;
	$count=0;
	
	/*foreach($_SESSION['shopping_cart'] as $key=>$product_id)
	{
		$product_array=category::productDetail($key);
		$nvpstr.="&L_PAYMENTREQUEST_0_NAME".$count."=".urlencode($product_array['title']) ;
		$nvpstr.="&L_PAYMENTREQUEST_0_QTY".$count."=".$product_id;
		$nvpstr.="&L_PAYMENTREQUEST_0_DESC".$count."=".urlencode($product_array['description']);
		$nvpstr.="&L_PAYMENTREQUEST_0_AMT".$count."=".$product_array['price'];
		$count++;
	}*/
	...
	If an error occured, show the resulting errors
	*/
	$resArray=hash_call($paymentmethod, $nvpstr);
...
function CreateRecurringPaymentsProfile()
{
	...
	$nvpstr.="&DESC=".urlencode($_SESSION['Package_Description']);
	$nvpstr.="&BILLINGPERIOD=Month";
	$nvpstr.="&BILLINGFREQUENCY=1";
	$nvpstr.="&AMT=".$_SESSION['Recurring_Amount'];
...
$countryCode, $currencyCode,$planname )
{
