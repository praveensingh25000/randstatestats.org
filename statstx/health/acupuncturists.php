...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: acupuncturistsData.php');
		exit;
	}
}
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>		
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="">
					...
					<!-- form -->
					<input type="hidden" name="session_setter" value="acupuncturists">
					<div class="form-div">
						<p>
						   <span class="choose"><?php if(isset($lang['lbl_choose_county_cities'])){ echo $lang['lbl_choose_county_cities']; } ?> </span>
						</p>
						<div class="table-div">
					...
					$(document).ready(function() {							
						...
						$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, {		
							...
							onAdd: function (item) {
								...
								jQuery.ajax({
									url: URL_SITE+'/statstx/health/acupuncturistsAjax.php',
									type: 'post',
