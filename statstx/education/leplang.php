...
#live Site URL For This Page: http://tx.rand.org/stats/education/leplang.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: leplangData.php');
		exit;
	}
}
...
$resultData = $admin->getDistinctColumnValuesUniversal($tablesname,'County');
$allCounties = $dbDatabase->getAll($resultData);

foreach($allCounties as $key => $value){
	if($value['County']!=''){
		$filtervaluesJsonArray[] = array('id' => $value['County'], 'name' => $value['County']);
	}
}
	...
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="leplang">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_school_districts_lep_prof'])){ echo $lang['lbl_select_school_districts_lep_prof']; } ?></span>&nbsp;&nbsp;
				...
				$(document).ready(function() {	
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statstx/education/leplangAjax.php',
								type: 'post',
