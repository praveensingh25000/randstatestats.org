...
#live Site URL For This Page: http://tx.rand.org/stats/education/schoolfin.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: schoolfinData.php');
		exit;
	}
}
$dbid = 80;
	...
	<section id="inner-content" <?php if(isset($related_DB) && mysql_num_rows($related_DB) <= 0) { ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="schoolfin">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_school_districts'])){ echo $lang['lbl_select_school_districts']; } ?></span>&nbsp;&nbsp;
				...
				$(document).ready(function() {	
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statstx/education/schoolfinAjax.php',
								type: 'post',
