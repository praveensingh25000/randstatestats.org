...
$resultData = $admin->getDistinctColumnValuesUniversal($tableCounties , 'County');
$allCounties = $dbDatabase->getAll($resultData);

foreach($allCounties as $key => $value){
	$filtervaluesJsonArray[] = array('id' => $value['County'], 'name' => ucfirst(strtolower(trim($value['County']))));
}
				...
				$(document).ready(function() {	
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statstx/education/enrollraceAjax.php',
								type: 'post',
