...
if(isset($_REQUEST['states']) && $_REQUEST['states']!=''){
	...
	foreach($statesArray as $stateKey => $stateAlpaCode){
		...
		$dataSqlAllResult = $admin->searchLikeUniversalArray($tablesnamecounties , $columnsArray, " and School_Code LIKE '%00' order by District");
		...
		if(mysql_num_rows($dataSqlAllResult)>0){
			...
			while($areaDetail = mysql_fetch_assoc($dataSqlAllResult)){
				if(!in_array($areaDetail['District'], $alreadyArray)){
					$district_isd = strstr($areaDetail['District'], 'ISD',true);
					if(!empty($district_isd)){
						$data .= '<option value="'.$areaDetail['School_Code'].'">'.ucwords(strtolower($district_isd)).' ISD</option>';
					}else {
						$data .= '<option value="'.$areaDetail['School_Code'].'">'.ucwords(strtolower($areaDetail['District'])).'</option>';
					}
				}
