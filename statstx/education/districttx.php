...
#live Site URL For This Page: http://tx.rand.org/stats/education/district.tx.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: districttxData.php');
		exit;
	}
}
...
<section id="container">
	<section id="inner-content" <?php if(mysql_num_rows($related_DB)<=0){ ?> class="conatiner-full" <?php } ?>>
		...
		<div class="categorie-data">
			<?php if(isset($dbsource) && $dbsource!=''){ ?><p><strong>Summary: </strong> <?php echo stripslashes($dbsource); ?></p><?php } ?>
			...
			<?php if(!empty($dateupdated) || !empty($nextupdate)) { ?>			
			<div class="time-div">
				<p>
					<?php if(!empty($dateupdated)) { ?>
						<b>Updated:</b><?php echo date('M. d, Y', strtotime($dateupdated)); ?>
					<?php } ?>
					<?php if(isset($nextupdate) && $nextupdate!='0000-00-00') { ?>
						<span class="pL20"><b>Next update:</b> <?php echo date('M. d, Y', strtotime($nextupdate)); ?> </span>
					<?php } ?>
				</p>
			</div>
			<?php } ?>
			...
			<form method="post" id="frmPost" name="frmPost" action="" novalidate="novalidate">
				...
				<!-- form -->
				<input type="hidden" name="session_setter" value="districttx">
				<div class="form-div">
					<p>
					   <span class="choose"><?php if(isset($lang['lbl_select_district'])){ echo $lang['lbl_select_district']; } ?></span>&nbsp;&nbsp;
				...
				$(document).ready(function() {	
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statstx/education/districttx.ajax.php',
								type: 'post',
