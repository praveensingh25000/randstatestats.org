...
$accounttypeAll = $accounttypeStr='';

if(!empty($searchedfields['accounttype'])){
	
	foreach($searchedfields['accounttype'] as $accounttypes){
		$accounttypeAll.="'".$accounttypes."'".',';
	}
	$accounttypeStr = substr($accounttypeAll, 0, -1);
}

$sql = "select ".$columnstobefetched." from schoolfin, schoolfin_cats, counties_districts where 1 ".$fieldsStrWhere." and ".$tablesname.".CDS in (".$area.") and ".$tablesnamecat.".catcode in (".$category.") and ".$tablesname.".Cat2 in (".$cat2.") and ".$tablesname.".FinanceType IN (".$accounttypeStr.")  ".$joinStr." and ".$tableDistricts.".distcode in (".$area1.") limit 4000";
