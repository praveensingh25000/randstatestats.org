...
#live Site URL For This Page: http://tx.rand.org/stats/community/weather.html
********************************************/
...
$countiesResult = $admin->searchLikeUniversalArray($tablesname , array('State' => 'TX'), 'order by Cat1');
$alreadyCounty = array();
if(mysql_num_rows($countiesResult)>0){
	while($value = mysql_fetch_assoc($countiesResult)){
		if(!in_array($value['Cat1'],$alreadyCounty))
		$filtervaluesJsonArray[] = array('id' => $value['Cat1'], 'name' => $value['Cat1']);

		$alreadyCounty[] = $value['Cat1'];
	}
}
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statstx/community/weatherAjax.php',
								type: 'post',
