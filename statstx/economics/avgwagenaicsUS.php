...
#live Site URL For This Page: http://tx.rand.org/stats/popdemo/popestUSdet.html
********************************************/
...
if(isset($_POST['getresults'])){
	...
	if(isset($validity_on) && $validity_on == '0') {
		...
		header('location: avgwagenaicsData.php');
		exit;
	}
}
	...
	<section id="inner-content">
		...
		<div class="categorie-data">
			...
			<form method="post" id="frmPost" name="frmPost" action="">
				...
				<?php
				$sql = "select DISTINCT( Cat1) from avgwage2US order by Cat2";
				$resultOwnership = $dbDatabase->run_query($sql, $dbDatabase->conn);
				$ownershipAll = $dbDatabase->getAll($resultOwnership);	
				...
				$(document).ready(function() {							
					...
					$("#search_criteria_duplicates").tokenInput(<?php echo $filtervaluesJson; ?>, { preventDuplicates: true,
						...
						onAdd: function (item) {
							...
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statstx/economics/avgwagenaicsUS_ajax.php',
								type: 'post',
						...
						onDelete: function (item) {
							...
							jQuery('#submitButtons').hide();
							jQuery.ajax({
								url: '<?php echo URL_SITE; ?>/statstx/avgwagenaicsUS_ajax.php',
								type: 'post',
								data: 'states='+states,
								success: function(dataresult){
									loader_unshow();
